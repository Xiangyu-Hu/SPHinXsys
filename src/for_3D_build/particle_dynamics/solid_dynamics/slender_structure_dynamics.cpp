...
namespace SPH
{
	...
	namespace slender_structure_dynamics
	{
		...
		Real BarAcousticTimeStepSize::reduce(size_t index_i, Real dt)
		{
			...
			Real time_setp_0 = SMIN(sqrt(smoothing_length_ / ((acc_[index_i] + acc_prior_[index_i]).norm() + TinyReal)),smoothing_length_ / (c0_ + vel_[index_i].norm()));
			Real time_setp_1 = SMIN(sqrt(1.0 / (dangular_vel_dt_[index_i].norm() + TinyReal)),1.0 / (angular_vel_[index_i].norm() + TinyReal));
			Real time_setp_2 = smoothing_length_ * sqrt(rho0_ * (1.0 - nu_ * nu_) / E0_ /(2.0 + (Pi * Pi / 12.0) * (1.0 - nu_) * (1.0 + 1.5 * pow(smoothing_length_ / thickness_[index_i], 2))));
		...
		void BarStressRelaxationFirstHalf::initialization(size_t index_i, Real dt)
		{
			...
			{
				Matd F_gaussian_point = F_[index_i] + gaussian_point_y[i] * F_bending_[index_i] * thickness_[index_i] * 0.5+gaussian_point_x[i]*F_b_bending_[index_i]*width_[index_i]*0.5;
                Matd dF_gaussian_point_dt = dF_dt_[index_i] + gaussian_point_y[i] * dF_bending_dt_[index_i] * thickness_[index_i] * 0.5 + gaussian_point_x[i] * dF_b_bending_dt_[index_i] * width_[index_i] * 0.5;
      ...
      /*          resultant_stress.row(Dimensions - 1) = Vecd::Zero().transpose();
            ...
            global_b_moment_[index_i] = J * current_transformation_matrix.transpose() * resultant_b_moment * current_transformation_matrix * transformation_matrix_[index_i].transpose() * inverse_F.transpose() * transformation_matrix_[index_i];
		...
		void BarStressRelaxationSecondHalf::initialization(size_t index_i, Real dt)
		{
            ...
            pseudo_b_n_[index_i] = transformation_matrix_[index_i].transpose() * getVectorAfterThinStructureRotation(local_pseudo_b_n_0, rotation_b_[index_i] );
		...
		ConstrainBarBodyRegionAlongAxis::ConstrainBarBodyRegionAlongAxis(BodyPartByParticle &body_part, int axis)
			: BaseLocalDynamics<BodyPartByParticle>(body_part), BarDataSimple(sph_body_),
			  ...
			  dangular_vel_dt_(particles_->dangular_vel_dt_), rotation_b_(particles_->rotation_b_), angular_b_vel_(particles_->angular_b_vel_), dangular_b_vel_dt_(particles_->dangular_b_vel_dt_) {}
		//=================================================================================================//
