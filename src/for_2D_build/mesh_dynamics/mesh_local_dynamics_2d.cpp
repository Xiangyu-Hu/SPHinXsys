...
void WriteMeshFieldToPlt::update(std::ofstream &output_file)
{
    ...
    mesh_for_column_major(
        ...
        [&](const Array2i &global_index)
        {
            ...
            output_file << DataValueFromGlobalIndex(kernel_weight_.Data(), global_index, &mesh_data_, cell_package_index_.Data()) << " ";
            output_file << DataValueFromGlobalIndex(kernel_gradient_.Data(), global_index, &mesh_data_, cell_package_index_.Data())[0] << " ";
            output_file << DataValueFromGlobalIndex(kernel_gradient_.Data(), global_index, &mesh_data_, cell_package_index_.Data())[1] << " ";
            output_file << " \n";
        });
    output_file << " \n";
}
