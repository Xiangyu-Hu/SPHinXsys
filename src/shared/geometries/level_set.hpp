...
MultilevelLevelSet::MultilevelLevelSet(
    const ExecutionPolicy &ex_policy, BoundingBox tentative_bounds,
...
void MultilevelLevelSet::configOperationExecutionPolicy(const ExecutionPolicy &ex_policy,
                                                        KernelType *kernel)
{
    host_clean_interface_ = makeUnique<CleanInterface<ParallelPolicy, KernelType>>(
        *mesh_data_set_.back(), kernel, global_h_ratio_vec_.back());
    host_correct_topology_ = makeUnique<CorrectTopology<ParallelPolicy, KernelType>>(
        *mesh_data_set_.back(), kernel, global_h_ratio_vec_.back());

    device_clean_interface_ = nullptr;
    device_correct_topology_ = nullptr;
    clean_interface_ = std::bind(
        &CleanInterface<ParallelPolicy, KernelType>::exec, host_clean_interface_.get(), _1);
    correct_topology_ = std::bind(
        &CorrectTopology<ParallelPolicy, KernelType>::exec, host_correct_topology_.get(), _1);
}
//=================================================================================================//
template <class ExecutionPolicy, class KernelType>
                                         ...
                                         MeshWithGridDataPackagesType* coarse_data)
{
    ...
    cell_package_index_set_.push_back(
        mesh_data_set_[level]->cell_package_index_.DelegatedDataField(ex_policy));
    meta_data_cell_set_.push_back(
        mesh_data_set_[level]->meta_data_cell_.DelegatedDataField(ex_policy));
}
//=================================================================================================//
template <class ExecutionPolicy>
