...
void Integration1stHalf<Inner<>, RiemannSolverType, KernelCorrectionType>::interaction(size_t index_i, Real dt)
{
    ...
    for (size_t n = 0; n != inner_neighborhood.current_size_; ++n)
    {
        ...
        acceleration -= (p_[index_i] * correction_(index_i) + p_[index_j] * correction_(index_j)) * dW_ijV_j * e_ij;
    ...
    interaction(size_t index_i, Real dt)
{
    ...
    for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
    {
        ...
        for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
        {
            ...
            acceleration -= riemann_solver_k.AverageP(this->p_[index_i] * correction_k(index_i), p_k[index_j] * correction_(index_j)) *
                            2.0 * e_ij * dW_ijV_j;
