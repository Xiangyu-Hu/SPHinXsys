...
BaseIntegration<DataDelegationType>::BaseIntegration(BaseRelationType &base_relation)
    : LocalDynamics(base_relation.getSPHBody()), DataDelegationType(base_relation),
      ...
      p_(this->particles_->template registerStateVariableData<Real>("Pressure")),
      drho_dt_(this->particles_->template registerStateVariableData<Real>("DensityChangeRate")),
      ...
      correction_(this->particles_)
{
    for (size_t k = 0; k != this->contact_particles_.size(); ++k)
    {
        ...
        Fluid &contact_fluid = DynamicCast<Fluid>(this, this->contact_bodies_[k]->getBaseMaterial());
    ...
    : BaseIntegration<DataDelegateContact>(contact_relation)
{
    ...
    {
        Fluid &contact_fluid = DynamicCast<Fluid>(this, contact_bodies_[k]->getBaseMaterial());
