...
FluidViscousForceOnShell::FluidViscousForceOnShell(BaseContactRelation &contact_relation)
    : LocalDynamics(contact_relation.getSPHBody()), FluidShellContactData(contact_relation), Vol_(particles_->Vol_), vel_ave_(*particles_->AverageVelocity())
{
...
void FluidViscousForceOnShell::interaction(size_t index_i, Real dt)
{
    ...
    for (size_t k = 0; k < contact_configuration_.size(); ++k)
    {
        ...
        for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
        {
            ...
            force += 2.0 * mu_k * vel_derivative * contact_neighborhood.dW_ijV_j_[n] * Vol_i * particles_->thickness_[index_i];
        }
    }
...
void FluidAngularConservativeViscousForceOnShell::interaction(size_t index_i, Real dt)
{
    ...
    for (size_t k = 0; k < contact_configuration_.size(); ++k)
    {
        ...
        for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
        {
            ...
            force += eta_ij * contact_neighborhood.dW_ijV_j_[n] * contact_neighborhood.e_ij_[n] * Vol_i * particles_->thickness_[index_i];
        }
    }
