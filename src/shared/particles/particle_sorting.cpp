...
//=================================================================================================//
void MoveSortableParticleDeviceData::operator()(DeviceInt *index_permutation, DeviceInt size)
{
...
{
    DeviceInt *unsorted_id_device = base_particles_.unsorted_id_device_;
    DeviceInt *sorted_id_device = base_particles_.sorted_id_device_;
    DeviceInt total_real_particles = base_particles_.total_real_particles_;
...
//=================================================================================================//
void ParticleSorting::sortingParticleData(DeviceInt *begin, DeviceInt size, execution::ParallelSYCLDevicePolicy execution_policy)
{
...
SYCL_EXTERNAL DeviceInt DeviceRadixSort<ValueType>::split_count(bool bit, sycl::nd_item<1> &item)
{
    ...
    DeviceInt true_before = sycl::inclusive_scan_over_group(item.get_group(), static_cast<DeviceInt>(bit),
                                                         sycl::plus<>{}); // prefix sum over group

    // Total number of 0s
    DeviceInt false_totals = sycl::group_broadcast(item.get_group(), group_range - true_before,
                                                group_range - 1);
...
//=================================================================================================//
template <class ValueType>
DeviceInt DeviceRadixSort<ValueType>::get_digit(DeviceInt key, DeviceInt d, DeviceInt radix_bits)
{
    return (key >> d * radix_bits) & ((static_cast<DeviceInt>(1) << radix_bits) - 1);
}
//=================================================================================================//
template <class ValueType>
DeviceInt DeviceRadixSort<ValueType>::get_bit(DeviceInt key, DeviceInt b)
{
    return (key >> b) & 1;
}
//=================================================================================================//
template <class ValueType>
DeviceInt DeviceRadixSort<ValueType>::find_max_element(const DeviceInt *data, DeviceInt size, DeviceInt identity)
{
    DeviceInt result = identity;
    auto &sycl_queue = execution::executionQueue.getQueue();
    {
        sycl::buffer<DeviceInt> buffer_result(&result, 1);
...
//=================================================================================================//
template <class ValueType>
void DeviceRadixSort<ValueType>::resize(DeviceInt data_size, DeviceInt radix_bits, DeviceInt workgroup_size)
{
    ...
    radix_ = static_cast<DeviceInt>(1) << radix_bits;  // radix = 2^b
...
//=================================================================================================//
template <class ValueType>
sycl::event DeviceRadixSort<ValueType>::sort_by_key(DeviceInt *keys, ValueType *data, DeviceInt data_size, sycl::queue &queue, DeviceInt workgroup_size, DeviceInt radix_bits)
{
    ...
    const DeviceInt max_key = find_max_element(keys, data_size, 0ul) + (uniform_case_masking_ ? 1 : 0);
    const DeviceInt bits_max_key = std::floor(std::log2(max_key)) + 1.0;                                    // bits needed to represent max_key
    const DeviceInt length = max_key ? bits_max_key / radix_bits + (bits_max_key % radix_bits ? 1 : 0) : 1; // max number of radix digits
        ...
        auto buckets_event = queue.submit([&](sycl::handler &cgh)
                                          {
                                              ...
                                              auto local_buckets = sycl::local_accessor<DeviceInt>(radix_, cgh);
                                              ...
                                              cgh.parallel_for(kernel_range_, [=, radix=radix_](sycl::nd_item<1> item) {
                                                                   const DeviceInt workgroup = item.get_group_linear_id(),
                                                                   ...
                                                                   for (DeviceInt b = 1; b < radix_bits; ++b) {  // sorting remaining bits
                                                                   ...
                                                                   for (DeviceInt r = 0; r < radix; ++r)
                                                                       local_buckets[r] = 0;

                                                                   item.barrier(sycl::access::fence_space::local_space);
                                                                   {
                                                                       sycl::atomic_ref<DeviceInt, sycl::memory_order_relaxed, sycl::memory_scope_work_group,
        ...
        sort_event = queue.submit([&](sycl::handler &cgh)
                                  {
                                      ...
                                      cgh.parallel_for(kernel_range_, [=](sycl::nd_item<1> item) {
                                                           ...
                                                           if(item.get_global_id() < data_size) {
                                                               ...
                                                               const DeviceInt rank = item.get_local_id(), workgroup = item.get_group_linear_id();
                                                               const SortablePair number = data_swap_acc[workgroup_size * workgroup + rank];
                                                               const DeviceInt radix_digit = get_digit(number.first, digit, radix_bits);
