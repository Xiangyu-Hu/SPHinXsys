...
ShearIntegration<Inner<OneLevel, MaterialType, Parameters...>>::
    ...
    : BaseInteraction(inner_relation), ForcePriorCK(this->particles_, "ShearForce"),
      ...
      dv_hourglass_acc_(this->particles_->template registerStateVariable<Vecd>("HourglassAcceleration")),
      ...
      dv_scale_penalty_force_(this->particles_->template registerStateVariable<Real>("ScalePenaltyForce"))
{
    ...
    this->particles_->template addEvolvingVariable<Vecd>(dv_hourglass_acc_);
}
//====================================================================================//
template <class MaterialType, typename... Parameters>
...
ShearIntegration<Inner<OneLevel, MaterialType, Parameters...>>::InteractKernel::
    ...
    : BaseInteraction::InteractKernel(ex_policy, encloser),
      ...
      hourglass_acc_(encloser.dv_hourglass_acc_->DelegatedData(ex_policy)),
      vel_gradient_(encloser.dv_vel_gradient_->DelegatedData(ex_policy)),
      shear_stress_(encloser.dv_shear_stress_->DelegatedData(ex_policy)),
...
{
    Vecd sum_acc = Vecd::Zero();
    Vecd hourglass_acc_increment = Vecd::Zero();
    for (UnsignedInt n = this->FirstNeighbor(index_i); n != this->LastNeighbor(index_i); ++n)
    {
        ...
        sum_acc += (shear_stress_[index_i] + shear_stress_[index_j]) * dW_ijV_j * e_ij;
        ...
        hourglass_acc_increment += penalty_scale * G_ * v_ij_correction.dot(e_ij) * vec_r_ij *
                                   dW_ijV_j * dt / vec_r_ij.squaredNorm();
    }
    hourglass_acc_[index_i] += hourglass_acc_increment * Vol_[index_i];
    shear_force_[index_i] = sum_acc * Vol_[index_i] + hourglass_acc_[index_i] ;
}
//====================================================================================//
template <class MaterialType, typename... Parameters>
