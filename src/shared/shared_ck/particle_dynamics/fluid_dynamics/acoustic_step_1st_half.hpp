    ...
    InteractKernel::interact(size_t index_i, Real dt)
{
    Vecd force = Vecd::Zero();
    Real rho_dissipation(0);
    //Matd stress_tensor_i = degradeToMatd(stress_tensor_3D_[index_i]); //degradeToMatd需要sycl修饰
    for (UnsignedInt n = this->FirstNeighbor(index_i); n != this->LastNeighbor(index_i); ++n)
    {
        UnsignedInt index_j = this->neighbor_index_[n];
        Real dW_ijV_j = this->dW_ij(index_i, index_j) * Vol_[index_j];
        ...
        //UnsignedInt index_j = inner_neighborhood.j_[n];
        //Real dW_ijV_j = this->dW_ij(index_i, index_j) * Vol_[index_j];
        Vecd nablaW_ijV_j = this->dW_ij(index_i, index_j) * Vol_[index_j] * this->e_ij(index_i, index_j);
        //Matd stress_tensor_j = degradeToMatd(stress_tensor_3D_[index_j]); //degradeToMatd需要sycl修饰
