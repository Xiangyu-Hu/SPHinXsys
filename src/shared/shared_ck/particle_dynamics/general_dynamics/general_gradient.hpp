...
Derivative<Base, OperatorType, DataType, KernelCorrectionType, RelationType<Parameters...>>::
    Derivative(RelationType<Parameters...> &relation, const std::string &variable_name)
    : BaseDynamicsType(relation), variable_name_(variable_name),
      ...
      dv_derivative_(this->particles_->template registerStateVariable<DerivativeType>(
          variable_name + getDerivativeName(OperatorType()), ZeroData<DerivativeType>::value)),
      kernel_correction_(this->particles_) {}
//=================================================================================================//
template <typename OperatorType, typename DataType, typename KernelCorrectionType, typename... Parameters>
Derivative<OperatorType, DataType, KernelCorrectionType, Inner<Parameters...>>::
    Derivative(Inner<Parameters...> &relation, const std::string &variable_name)
    : BaseDynamicsType(relation, variable_name) {}
//=================================================================================================//
template <typename OperatorType, typename DataType, typename KernelCorrectionType, typename... Parameters>
template <class ExecutionPolicy, class EncloserType>
Derivative<OperatorType, DataType, KernelCorrectionType, Inner<Parameters...>>::InteractKernel::
    InteractKernel(const ExecutionPolicy &ex_policy, EncloserType &encloser)
    : BaseDynamicsType::InteractKernel(ex_policy, encloser),
      zero_derivative_(ZeroData<DerivativeType>::value),
      Vol_(encloser.dv_Vol_->DelegatedData(ex_policy)),
      variable_(encloser.dv_variable_->DelegatedData(ex_policy)),
      derivative_(encloser.dv_derivative_->DelegatedData(ex_policy)),
      correction_(ex_policy, encloser.kernel_correction_) {}
//=================================================================================================//
template <typename OperatorType, typename DataType, typename KernelCorrectionType, typename... Parameters>
void Derivative<OperatorType, DataType, KernelCorrectionType, Inner<Parameters...>>::
    InteractKernel::interact(size_t index_i, Real dt)
{
    DerivativeType summation = zero_derivative_;
    for (UnsignedInt n = this->FirstNeighbor(index_i); n != this->LastNeighbor(index_i); ++n)
    {
        UnsignedInt index_j = this->neighbor_index_[n];
        Vecd corrected_gradW_ijV_j = this->dW_ij(index_i, index_j) * Vol_[index_j] *
                                     correction_(index_i) * this->e_ij(index_i, index_j);
        DataType difference = variable_[index_i] - variable_[index_j];
        summation -= operator_(difference, corrected_gradW_ijV_j);
    }
    this->derivative_[index_i] = summation;
}
//=================================================================================================//
template <typename OperatorType, typename DataType, typename KernelCorrectionType, typename... Parameters>
Derivative<OperatorType, DataType, KernelCorrectionType, Contact<Parameters...>>::
    Derivative(Contact<Parameters...> &relation, const std::string &variable_name)
    : BaseDynamicsType(relation, variable_name)
{
...
template <typename OperatorType, typename DataType, typename KernelCorrectionType, typename... Parameters>
template <class ExecutionPolicy, class EncloserType>
Derivative<OperatorType, DataType, KernelCorrectionType, Contact<Parameters...>>::InteractKernel::
...
//=================================================================================================//
template <typename OperatorType, typename DataType, typename KernelCorrectionType, typename... Parameters>
void Derivative<OperatorType, DataType, KernelCorrectionType, Contact<Parameters...>>::
    InteractKernel::interact(size_t index_i, Real dt)
{
    DerivativeType summation = zero_derivative_;
    for (UnsignedInt n = this->FirstNeighbor(index_i); n != this->LastNeighbor(index_i); ++n)
    {
        ...
        Vecd corrected_gradW_ijV_j = this->dW_ij(index_i, index_j) * contact_Vol_[index_j] *
                                     correction_(index_i) * this->e_ij(index_i, index_j);
        DataType difference = variable_[index_i] - contact_variable_[index_j];
        summation -= operator_(difference, corrected_gradW_ijV_j);
    }
    this->derivative_[index_i] += summation;
}
...
Hessian<Base, DataType, RelationType<Parameters...>>::Hessian(
    RelationType<Parameters...> &relation, const std::string &variable_name)
    : BaseDynamicsType(relation), variable_name_(variable_name),
      dv_variable_(this->particles_->template getVariableByName<DataType>(variable_name)),
