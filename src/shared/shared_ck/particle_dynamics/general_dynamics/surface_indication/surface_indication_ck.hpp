...
{
    // Check if the particle is near a surface
    ...
    for (UnsignedInt n = this->FirstNeighbor(index_i); n != this->LastNeighbor(index_i); ++n)
    {
        ...
        // If a neighbor is under the threshold and within the smoothing length, mark near-surface
    ...
    // Check if the particle is near a previously marked surface
    // Equvilent to SpatialTemporal
    ...
    // If the current particle is under the threshold but not near any surface (current or previous),
    // artificially increase its pos_div to avoid misclassifying it as surface
    ...
    // Determine the new indicator for this particle
    int new_indicator = 1;  // Default: surface
    // If the local pos_div is above the threshold and it is not near-surface, mark as not surface
