...
PairwiseDissipation<Inner<Splitting, DissipationType, SourceTermType, Parameters...>>::
    PairwiseDissipation(
        Inner<Parameters...> &inner_relation, const std::string &variable_name, Args &&...args)
    : BaseDissipationType(inner_relation, variable_name),
      source_term_model_(this->particles_, std::forward<Args>(args)...) {}
//=================================================================================================//
template <typename DissipationType, typename SourceTermType, typename... Parameters>
template <class ExecutionPolicy, class EncloserType>
PairwiseDissipation<Inner<Splitting, DissipationType, SourceTermType, Parameters...>>::
    ...
    : BaseDissipationType::InteractKernel(ex_policy, encloser),
      ...
      source_term_(ex_policy, encloser.source_term_model_) {}
//=================================================================================================//
template <typename DissipationType, typename SourceTermType, typename... Parameters>
void PairwiseDissipation<Inner<Splitting, DissipationType, SourceTermType, Parameters...>>::
    InteractKernel::interact(size_t index_i, Real dt)
{
    Real split_dt = dt * 0.5;
    std::array<Real, MaximumNeighborhoodSize> parameter_b;
    Real Vol_i = this->Vol_[index_i];
    DataType variable_i = this->variable_[index_i] + source_term_(index_i) * split_dt;
    // forward sweep
    for (UnsignedInt n = this->FirstNeighbor(index_i); n != this->LastNeighbor(index_i); ++n)
    {
        UnsignedInt index_j = this->neighbor_index_[n];
    ...
    // backward sweep
    variable_i += source_term_(index_i) * split_dt;
