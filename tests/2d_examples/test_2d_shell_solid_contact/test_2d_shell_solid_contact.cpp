...
/**
 * @file 	test_2d_shell_solid.cpp
 * @brief 	This is the case file for the test of shell-solid contact.
 * @author   Xiangyu Hu
 */
...
const BoundingBox system_domain_bounds(Vec2d(-half_length_plate, -thickness_plate), Vec2d(half_length_plate, radius_shell));
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
class Plate : public MultiPolygonShape
{
  public:
    explicit Plate(const std::string &shape_name) : MultiPolygonShape(shape_name)
    {
        Vecd plate_tranlation(0, -0.5 * thickness_plate);
        multi_polygon_.addABox(Transform(plate_tranlation), Vec2d(half_length_plate, 0.5 * thickness_plate), ShapeBooleanOps::add);
    }
};
class ShellParticleGenerator : public ParticleGenerator<Surface>
{
  public:
    explicit ShellParticleGenerator(SPHBody &sph_body) : ParticleGenerator<Surface>(sph_body){};
...
// handle command line arguments
    ...
    auto shell_particle_generator = shell.makeSelfDefined<ShellParticleGenerator>();
    shell.generateParticles(shell_particle_generator);
    ...
    plate.defineParticlesAndMaterial<ElasticSolidParticles, SaintVenantKirchhoffSolid>(rho0_s, Youngs_modulus_plate, poisson);
    plate.generateParticles<Lattice>();
    //----------------------------------------------------------------------
    ...
    SurfaceContactRelationFromShell shell_plate_contact(shell, {&plate});
    SurfaceContactRelationToShell plate_shell_contact(plate, {&shell}, {true});
    ...
    // solid
    InteractionWithUpdate<LinearGradientCorrectionMatrixInner> plate_corrected_configuration(plate_inner);
    ReduceDynamics<solid_dynamics::AcousticTimeStepSize> plate_get_time_step_size(plate);
    Dynamics1Level<solid_dynamics::Integration1stHalfPK2> plate_stress_relaxation_first_half(plate_inner);
    Dynamics1Level<solid_dynamics::Integration2ndHalf> plate_stress_relaxation_second_half(plate_inner);
    DampingWithRandomChoice<InteractionSplit<DampingPairwiseInner<Vec2d>>>
        plate_damping(0.5, plate_inner, "Velocity", physical_viscosity);
    BoundaryGeometry plate_boundary_geometry(plate, "BoundaryGeometry");
    SimpleDynamics<solid_dynamics::FixBodyPartConstraint> plate_constrain(plate_boundary_geometry);
    // contact
    ...
    InteractionWithUpdate<solid_dynamics::ContactForce> plate_compute_solid_contact_forces(plate_shell_contact);
    //-----------------------------------------------------------------------------
    //	outputs
    //-----------------------------------------------------------------------------
    ...
    while (GlobalStaticVariables::physical_time_ < end_time)
    {
        ...
        while (integration_time < output_interval)
        {
            ...
            while (relaxation_time < Dt)
            {
                ...
                dt = std::min(shell_time_step_size.exec(), plate_get_time_step_size.exec());

                shell_stress_relaxation_first.exec(dt);
                plate_stress_relaxation_first_half.exec(dt);
                ...
                plate_stress_relaxation_second_half.exec(dt);
