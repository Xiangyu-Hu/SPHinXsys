...
//----------------------------------------------------------------------
Real cylinder_radius = 0.01;                             /**< Cylinder radius. */
Real DL = 400.0 * cylinder_radius;                          /**< Water tank length. */
...
{
    // 2D旋转矩阵的转置（全局→局部坐标系）
    // 局部X方向力 = F_global_x * cosθ + F_global_y * sinθ
    Real cos_theta = std::cos(rotation_angle);
    Real sin_theta = std::sin(rotation_angle);
    return global_force[0] * cos_theta + global_force[1] * sin_theta;
}
//----------------------------------------------------------------------
...
int main(int ac, char *av[])
{
    ...
    ContactRelation wetting_observer_contact(wetting_observer, {&cylinder});
        ...
        // 新增：前段中心观测体与圆柱的接触关系（仿照OWSC的observer_contact_with_flap）
 ...
 // 修正：创建圆柱初始速度设置的动力学对象
    ...
    InteractionWithUpdate<solid_dynamics::PressureForceFromFluid<decltype(fluid_density_relaxation)>> pressure_force_from_fluid(cylinder_contact);
     ...
     // 仿照2d_flow_around_cylinder，定义全局坐标系总粘性力/压力力的统计
    ...
    //仿照OWSC的interpolation_observer_position，插值更新前段中心观测点位置
