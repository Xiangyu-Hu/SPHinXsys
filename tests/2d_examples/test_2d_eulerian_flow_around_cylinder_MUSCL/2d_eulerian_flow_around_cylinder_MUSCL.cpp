...
class SolidInitialCondition : public LocalDynamics
{
  public:
    explicit SolidInitialCondition(SPHBody &sph_body)
        : LocalDynamics(sph_body),
          vel_(particles_->registerStateVariable<Vecd>("Velocity")){};
    virtual ~SolidInitialCondition(){};
...
int main(int ac, char *av[])
{
    ...
    // ensure Velocity state is registered for Eulerian compressible MUSCL path
    water_block.getBaseParticles().registerStateVariable<Vecd>("Velocity");
    // NormalDirection needed by NormalDirectionFromBodyShape on fluid body
    water_block.getBaseParticles().registerStateVariable<Vecd>("NormalDirection");
    // DistanceFromWall needed by DistanceFromWall dynamics
    water_block.getBaseParticles().registerStateVariable<Vecd>("DistanceFromWall");
    // Pressure needed by compressible dynamics
    ...
    // NormalDirection for cylinder shape processing
    cylinder.getBaseParticles().registerStateVariable<Vecd>("NormalDirection");
    // Pressure for wall contact terms (wall_p_ access)
    cylinder.getBaseParticles().registerStateVariable<Real>("Pressure");
    //----------------------------------------------------------------------
    ...
    // gradients for MUSCL (inner-only for Aşama 1)
    InteractionWithUpdate<fluid_dynamics::DensityGradient<Inner<LinearGradientCorrection>>> density_gradient(water_block_inner);
    InteractionWithUpdate<fluid_dynamics::VelocityGradientWithWall<LinearGradientCorrection>> velocity_gradient(water_block_inner, water_block_contact);
    InteractionWithUpdate<fluid_dynamics::PressureGradientWithWall<LinearGradientCorrection>> pressure_gradient(water_block_inner, water_block_contact);
    SimpleDynamics<NormalDirectionFromBodyShape> cylinder_normal_direction(cylinder);
    SimpleDynamics<SolidInitialCondition> cylinder_initial_condition(cylinder);
    InteractionDynamics<fluid_dynamics::DistanceFromWall> distance_from_wall(water_block_contact);

    // MUSCL + HLLC (with wall) - working configuration
    InteractionWithUpdate<fluid_dynamics::EulerianCompressibleIntegration1stHalfMUSCLWithWall> pressure_relaxation(water_block_inner, water_block_contact);
    InteractionWithUpdate<fluid_dynamics::EulerianCompressibleIntegration2ndHalfMUSCLWithWall> density_relaxation(water_block_inner, water_block_contact);

    InteractionWithUpdate<fluid_dynamics::ViscousForceWithWall> viscous_force(water_block_inner, water_block_contact);
    SimpleDynamics<NormalDirectionFromBodyShape> water_block_normal_direction(water_block);
    ReduceDynamics<fluid_dynamics::EulerianCompressibleAcousticTimeStepSize> get_fluid_time_step_size(water_block, 0.5);
    ...
    // PressureForceFromFluid kapalı (FSI uyumlu HLLC henüz eklenmedi)
    ...
    cylinder_initial_condition.exec();
    distance_from_wall.exec();
