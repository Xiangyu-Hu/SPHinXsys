...
class PressureForceFromFluidInFVM : public BaseForceFromFluidInFVM
{
  ...
  public:
    explicit PressureForceFromFluidInFVM(BaseInnerRelation &inner_relation, vector<vector<size_t>> each_boundary_type_contact_real_index)
        : BaseForceFromFluidInFVM(inner_relation), fluid_(DynamicCast<WeaklyCompressibleFluid>(this, particles_->getBaseMaterial())), vel_(*particles_->getVariableByName<Vecd>("Velocity")),
          p_(*particles_->getVariableByName<Real>("Pressure")), rho_(*particles_->getVariableByName<Real>("Density")), riemann_solver_(fluid_, fluid_),
