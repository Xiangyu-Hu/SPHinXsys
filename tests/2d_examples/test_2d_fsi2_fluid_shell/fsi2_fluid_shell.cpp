...
#include "fsi2_fluid_shell.h" //	case file to setup the test case
using namespace SPH;
...
int main(int ac, char *av[])
{
    ...
    SolidBody circle(sph_system, makeShared<Circle>("Circle"));
    circle.defineAdaptationRatios(1.15, 2.0);
    circle.defineBodyLevelSetShape()->writeLevelSet(io_environment);
    circle.defineParticlesAndMaterial<SolidParticles, Solid>();
    (!sph_system.RunParticleRelaxation() && sph_system.ReloadParticles())
        ? circle.generateParticles<ParticleGeneratorReload>(io_environment, circle.getName())
        : circle.generateParticles<ParticleGeneratorLattice>();
    ...
    if (sph_system.RunParticleRelaxation())
    {
        ...
        //----------------------------------------------------------------------
        InnerRelation circle_inner(circle);
        //----------------------------------------------------------------------
        //	Methods used for particle relaxation.
        //----------------------------------------------------------------------
        /** Random reset the insert body particle position. */
        SimpleDynamics<RandomizeParticlePosition> random_circle_particles(circle);
        /** Write the body state to Vtp file. */
        BodyStatesRecordingToVtp write_circle_to_vtp(io_environment, {&circle});
        /** Write the particle reload files. */
        ReloadParticleIO write_particle_reload_files(io_environment, {&circle});
        /** A  Physics relaxation step. */
        relax_dynamics::RelaxationStepInner relaxation_step_inner(circle_inner);
        //----------------------------------------------------------------------
        //	Particle relaxation starts here.
        //----------------------------------------------------------------------
        random_circle_particles.exec(0.25);
        relaxation_step_inner.SurfaceBounding().exec();
        write_circle_to_vtp.writeToFile(0);
        //----------------------------------------------------------------------
        ...
        while (ite_p < 1000)
        {
            ...
            if (ite_p % 200 == 0)
            {
                ...
                write_circle_to_vtp.writeToFile(ite_p);
            }
        }
    ...
    ComplexRelation water_block_complex(water_block, RealBodyVector{&wall_boundary, &circle, &beam});
    ...
    SimpleDynamics<NormalDirectionFromBodyShape> circle_normal_direction(circle);
