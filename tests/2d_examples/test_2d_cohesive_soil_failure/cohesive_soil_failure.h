...
//----------------------------------------------------------------------
template <typename... InteractionTypes>
class CornerIndication;

template <class DataDelegationType>
class CornerIndication<DataDelegationType>
    : public LocalDynamics, public DataDelegationType
{
  public:
    template <class BaseRelationType>
    explicit CornerIndication(BaseRelationType &base_relation)
        : LocalDynamics(base_relation.getSPHBody()), DataDelegationType(base_relation),
          corner_indicator_(particles_->registerStateVariable<int>("CornerIndicator")),
          Vol_(particles_->getVariableDataByName<Real>("VolumetricMeasure")),
          pos_div_(particles_->registerStateVariable<Real>("PositionDivergence")),
          pos_(particles_->getVariableDataByName<Vecd>("Position")),
          threshold_by_dimensions_(0.75 * Dimensions)
    {
        particles_->addVariableToSort<int>("CornerIndicator");
    }
    virtual ~CornerIndication(){};

  protected:
    int *corner_indicator_;
    Real *Vol_, *pos_div_;
    Vecd *pos_;
...
template <>
class CornerIndication<Inner<>>
    : public CornerIndication<DataDelegateInner>
{
  public:
    explicit CornerIndication(BaseInnerRelation &inner_relation)
        : CornerIndication<DataDelegateInner>(inner_relation){};
    virtual ~CornerIndication(){};
    void interaction(size_t index_i, Real dt = 0.0)
    {
        Real pos_div = 0.0;
        corner_indicator_[index_i] = 0;
        const Neighborhood &inner_neighborhood = inner_configuration_[index_i];
        for (size_t n = 0; n != inner_neighborhood.current_size_; ++n)
        {
            size_t index_j = inner_neighborhood.j_[n];
        ...
        if (pos_div < 0.55 * Dimensions)
            corner_indicator_[index_i] = 1;
    };
};

// template <>
// class CornerIndication<Contact<>>
//     : public CornerIndication<DataDelegateContact>
// {
//   public:
//     explicit CornerIndication(BaseContactRelation &contact_relation)
//         : CornerIndication<DataDelegateContact>(contact_relation){};
//     virtual ~CornerIndication(){};
//     void interaction(size_t index_i, Real dt = 0.0)
//     {
//         Real pos_div = 0.0;
