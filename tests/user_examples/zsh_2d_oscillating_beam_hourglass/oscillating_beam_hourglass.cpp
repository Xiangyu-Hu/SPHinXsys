...
//------------------------------------------------------------------------------
Real CFL_acoustic = 0.4;
Real CFL_advection = 0.2;
...
int main(int ac, char* av[])
{
    ...
    InteractionWithUpdate<continuum_dynamics::ShearStressRelaxationHourglassControl> beam_shear_stress_relaxation(beam_body_inner, if_hourglass_control);
    Dynamics1Level<fluid_dynamics::Integration2ndHalfInnerDissipativeRiemann> beam_density_relaxation(beam_body_inner);
    InteractionWithUpdate<continuum_dynamics::ShearStressRelaxation2ndHalf> beam_shear_stress_rate_relaxation(beam_body_inner);
    InteractionDynamics<continuum_dynamics::ArtificialNormalShearStressRelaxation> beam_artificial_normal_shear_stress_relaxation(beam_body_inner);
    ReduceDynamics<continuum_dynamics::ContinuumAcousticTimeStepSize> computing_time_step_size(beam_body);

    ReduceDynamics<fluid_dynamics::AdvectionTimeStepSize> fluid_advection_time_step(beam_body, U_max, CFL_advection);
    ReduceDynamics<fluid_dynamics::AcousticTimeStepSize> fluid_acoustic_time_step(beam_body, CFL_acoustic);
