...
int main(int ac, char *av[])
{
	...
	/** Emitter setup and buffer inflow condition. */
	...
	/** Free-stream BC. */
	InteractionWithUpdate<fluid_dynamics::SpatialTemporalFreeSurfaceIdentificationComplex> free_stream_surface_indicator(water_shell_complex);
	InteractionWithUpdate<fluid_dynamics::DensitySummationFreeStreamComplex> update_fluid_density(water_shell_complex);
	...
	Dynamics1Level<fluid_dynamics::FluidShellIntegration1stHalfRiemann> pressure_relaxation(water_shell_complex);
	pressure_relaxation.post_processes_.push_back(&velocity_boundary_condition_constraint);
	Dynamics1Level<fluid_dynamics::FluidShellIntegration2ndHalfRiemann> density_relaxation(water_shell_complex);
	InteractionDynamics<fluid_dynamics::ViscousAccelerationWithShell> viscous_acceleration(water_shell_complex);
	InteractionDynamics<fluid_dynamics::TransportVelocityCorrectionComplex> transport_velocity_correction(water_shell_complex);
	...
	ReducedQuantityRecording<ReduceDynamics<solid_dynamics::TotalForceOnSolid>>
		write_total_force_on_sehll(io_environment, shell_baffle);
	...
	write_total_force_on_sehll.writeToFile();
	/** Time parameters. */
