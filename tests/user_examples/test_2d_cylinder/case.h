...
Real particle_spacing_ref = 0.2;				/**< Global reference resolution. */
...
Real radius_mid_surface = insert_circle_radius - thickness / 2.0;	
int particle_number_mid_surface = 2 * radius_mid_surface * Pi / particle_spacing_ref;
...
public:
	...
	virtual void initializeGeometricVariables() override
	{
		...
		{
			Real x = radius_mid_surface * sin( i * 2 * Pi / (Real)particle_number_mid_surface);
			Real y = radius_mid_surface * cos( i * 2 * Pi / (Real)particle_number_mid_surface);
            initializePositionAndVolumetricMeasure(Vecd(insert_circle_center[0] + x, insert_circle_center[1] + y), particle_spacing_ref);
