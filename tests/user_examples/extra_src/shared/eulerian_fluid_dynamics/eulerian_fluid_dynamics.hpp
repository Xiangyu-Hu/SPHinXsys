...
void EulerianIntegration1stHalfConsistency<RiemannSolverType>::interaction(size_t index_i, Real dt)
{
    ...
    for (size_t n = 0; n != inner_neighborhood.current_size_; ++n)
    {
        ...
        Matd rho_vel_sqr_ave = 0.5 * (rho_i * vel_i * vel_i.transpose() + rho_j * vel_j * vel_j.transpose());
...
void EulerianIntegration1stHalfWithWallConsistency<EulerianIntegration1stHalfType>::interaction(size_t index_i, Real dt)
{
    ...
    for (size_t k = 0; k < FluidWallData::contact_configuration_.size(); ++k)
    {
        ...
        for (size_t n = 0; n != wall_neighborhood.current_size_; ++n)
        {
            ...
            Matd rho_vel_sqr_ave = 0.5 * (rho_i * vel_i * vel_i.transpose() + rho_in_wall * vel_in_wall * vel_in_wall.transpose());
            Matd p_ave = (rho_i * p_i + rho_in_wall * p_in_wall) / (rho_i + rho_in_wall) * Matd::Identity();
            Matd rho_dissipation = rho_ave * (2 * (rho_i * vel_i + rho_in_wall * vel_in_wall) / (rho_i + rho_in_wall) * (this->riemann_solver_.DissipativeUJump(p_jump, u_jump)).transpose() +
                                              (this->riemann_solver_.DissipativeUJump(p_jump, u_jump)) * (this->riemann_solver_.DissipativeUJump(p_jump, u_jump)).transpose());
