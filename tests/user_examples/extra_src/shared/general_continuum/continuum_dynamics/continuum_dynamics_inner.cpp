...
ShearStressRelaxationHourglassControl ::
    ...
    : BaseRelaxation(inner_relation),
      ...
      B_(*particles_->getVariableByName<Matd>("KernelCorrectionMatrix")),
      pos0_(particles_->pos0_), hourglass_control_(hourglass_control),
...
void ShearStressRelaxationHourglassControlJ2Plasticity::interaction(size_t index_i, Real dt)
{
    ...
    Mat3d shear_stress_old = shear_stress_3D_[index_i];
    ...
    shear_strain_[index_i] += (strain_rate - isotropic_strain_rate * Matd::Identity()) * dt;
    scale_coef_[index_i] = reduceTensor(shear_stress_3D_[index_i]) * shear_strain_[index_i] *
                           (shear_strain_[index_i] * shear_strain_[index_i].transpose() + 1.0e-6 * Matd::Identity()).inverse();
}
