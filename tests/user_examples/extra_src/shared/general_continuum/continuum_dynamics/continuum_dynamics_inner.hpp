...
namespace SPH
{
	namespace continuum_dynamics
	{
		...
		BaseStressRelaxation1stHalf<RiemannSolverType>::BaseStressRelaxation1stHalf(BaseInnerRelation& inner_relation)
			...
			velocity_gradient_(particles_->velocity_gradient_), acc_deviatoric_plastic_strain_(particles_->acc_deviatoric_plastic_strain_), vertical_stress_(particles_->vertical_stress_), E_(plastic_continuum_.getYoungsModulus()), nu_(plastic_continuum_.getPoissonRatio()), B_(*particles_->getVariableByName<Matd>("KernelCorrectionMatrix")) {}
		//=================================================================================================//
		template <class RiemannSolverType>
		...
		void BaseStressRelaxation1stHalf<RiemannSolverType>::interaction(size_t index_i, Real dt)
		{
			...
			for (size_t n = 0; n != inner_neighborhood.current_size_; ++n)
			{
				...
				Matd velocity_gradient_ij = -(vel_[index_i] - vel_[index_j]) * (B_[index_i] * e_ij * dW_ijV_j).transpose();
		...
		//=================================================================================================//
		template <class RiemannSolverType>
		Vecd BaseStressRelaxation2ndHalf<RiemannSolverType>::computeNonConservativeAcceleration(size_t index_i)
		{
		...
		{
			Real G_ = plastic_continuum_.getShearModulus(plastic_continuum_.getYoungsModulus(), plastic_continuum_.getPoissonRatio());
			...
			acc_[index_i] += acceleration + acc_hourglass_[index_i];
		...
		template <class RiemannSolverType>
		BaseStressRelaxation3rdHalf<RiemannSolverType>::BaseStressRelaxation3rdHalf(BaseInnerRelation& inner_relation)
			: BaseRelaxationPlastic(inner_relation), riemann_solver_(plastic_continuum_, plastic_continuum_),
			velocity_gradient_(particles_->velocity_gradient_), Vol_(particles_->Vol_), mass_(particles_->mass_), E_(plastic_continuum_.getYoungsModulus()), nu_(plastic_continuum_.getPoissonRatio()) {}
		//=================================================================================================//
		template <class RiemannSolverType>
		void BaseStressRelaxation3rdHalf<RiemannSolverType>::initialization(size_t index_i, Real dt)
		{
			pos_[index_i] += vel_[index_i] * dt * 0.5;
		}
		//=================================================================================================//
		template <class RiemannSolverType>
		void BaseStressRelaxation3rdHalf<RiemannSolverType>::interaction(size_t index_i, Real dt)
		{
		...
		//=================================================================================================//
		template <class RiemannSolverType>
		void BaseStressRelaxation3rdHalf<RiemannSolverType>::update(size_t index_i, Real dt)
		{
			rho_[index_i] += drho_dt_[index_i] * dt * 0.5;
