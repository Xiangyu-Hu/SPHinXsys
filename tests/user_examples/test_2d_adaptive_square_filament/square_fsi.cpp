...
int main(int ac, char *av[])
{
	...
	shell_baffle.defineAdaptationRatios(1.15, 2.0);
	...
	AdaptiveContactRelation shell_water_contact(shell_baffle, {&water_block});
	...
	if (sph_system.run_particle_relaxation_)
	{
		...
		ComplexRelation water_complex(water_inner, {&wall_boundary, &shell_baffle});
		...
		relax_dynamics::RelaxationStepComplex relaxation(water_complex, "OuterBoundary", true);
		...
		relaxation.surface_bounding_.parallel_exec();
	...
	InteractionWithUpdate<fluid_dynamics::DensitySummationFreeStreamComplexAdaptive> update_fluid_density(water_wall_contact, water_shell_complex);
	...
	InteractionDynamics<fluid_dynamics::TransportVelocityCorrectionComplexAdaptive> transport_velocity_correction(water_wall_contact, water_shell_complex);
