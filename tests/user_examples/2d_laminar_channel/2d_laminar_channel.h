...
//----------------------------------------------------------------------
//	Get total acceleration including inner pressure- or stress-induced acceleration and other accelerations
//----------------------------------------------------------------------
class getTotalAcc : public fluid_dynamics::FluidInitialCondition
{
	StdLargeVec<Vecd> &acc_;
	StdLargeVec<Vecd> acc_copied_;
public:
	getTotalAcc(SPHBody& sph_body) : FluidInitialCondition(sph_body),
		acc_(*particles_->getVariableByName<Vecd>("Acceleration"))
	{
		particles_->registerVariable(acc_copied_, "AccelerationCopied");
		particles_->addVariableToWrite<Vecd>("AccelerationCopied");
	}
