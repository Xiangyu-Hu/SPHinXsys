...
const Real shell_thickness = 0.055 * unit_scale;         /**<Balloon thickness. */
...
//----------------------------------------------------------------------
const Real rho0_s = 1150.0;
const Real youngs_modulus = 12 * 13.1e6;
...
static constexpr double mmHg_to_Pa = 133.322;
const double frequency_bpm = 1250;                          // beats per minute
const double time_cycle = 60 / frequency_bpm;               // seconds per beat: 48ms
const double duration_deflation = 18.0 / 30.0 * time_cycle; // deflate: 10/30, hold: 8/30
const double t_pressure_0 = 1.6 / 18.0;                     // 2ms /duration_deflation
const double t_target_pressure =
    12.0 / 18.0; // from pressure 0 to target pressure: 15ms / duration_deflation
// const double t_shift_deflation = 0.005 / duration_deflation; // original: 3.0 / 18.0;  now 5 ms
const double t_shift_deflation = 0;
const double pressure_last = 0;
const double pressure_target =
    300 * mmHg_to_Pa * 0.8; // actually using: -300 mmHg but leave it lower to reach max deflation
...
class BalloonForce : public thin_structure_dynamics::ConstrainShellBodyRegion
{
  ...
  protected:
    ...
    StdLargeVec<Vecd> &n_;
    inline Real get_pressure(Real t_ratio, Real t_shifted_i)
    {
        if (t_ratio < t_shifted_i)
            return pressure_last;
        if (t_ratio < t_pressure_0 + t_shifted_i)
            return pressure_last * (t_shifted_i + t_pressure_0 - t_ratio) / t_pressure_0;
        if (t_ratio < t_pressure_0 + t_target_pressure + t_shifted_i)
            return pressure_target * (1 - (t_shifted_i + t_target_pressure + t_pressure_0 - t_ratio) / t_target_pressure);
        else
    ...
    {
        Real run_time = GlobalStaticVariables::physical_time_;
        Real t_ratio = run_time / duration_deflation;
        Real x_ratio_i = (pos0_[index_i].x() - (balloon_distal_end + balloon_fixed_length_distal)) / (balloon_full_length - balloon_fixed_length_proximal - balloon_fixed_length_distal);
        const double t_shifted_i = t_shift_deflation * x_ratio_i;
        Real pressure_i = get_pressure(t_ratio, t_shifted_i);
        force_prior_[index_i] = -pressure_i * n_[index_i] * particles_->Vol_[index_i] / shell_thickness;
    };
};
    ...
    auto run_self_contact = [&]()
    {
        while (GlobalStaticVariables::physical_time_ < end_time)
        {
            ...
            while (integration_time < output_interval)
            {
                ...
                dt_s = 0.5 * shell_time_step_size.exec();
