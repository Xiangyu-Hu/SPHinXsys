...
void configureParticleSpacing(Real spacing)
{
    if (spacing > fish_thickness / 4.0)
    {
        throw std::invalid_argument(
            "--dp must not exceed one quarter of the fish thickness "
            "(0.0075 m)");
    }
    particle_spacing_ref = spacing;
    DL_sponge = particle_spacing_ref * 20.0;
    BW = particle_spacing_ref * 4.0;
    buffer_halfsize = Vec2d(0.5 * DL_sponge, 0.5 * DH);
    buffer_translation = Vec2d(-DL_sponge, 0.0) + buffer_halfsize;
    emitter_halfsize = Vec2d(0.5 * BW, 0.5 * DH);
    emitter_translation = Vec2d(-DL_sponge, 0.0) + emitter_halfsize;
    emitter_buffer_halfsize = Vec2d(0.5 * DL_sponge, 0.5 * DH);
    emitter_buffer_translation =
        Vec2d(-DL_sponge, 0.0) + emitter_buffer_halfsize;
    disposer_halfsize = Vec2d(0.5 * BW, 0.75 * DH);
    disposer_translation =
        Vec2d(DL, DH + 0.25 * DH) - disposer_halfsize;
    fish_shape_resolution = particle_spacing_ref * 0.5;
}

Vec2d fishTailObservationPoint()
{
    const Real shape_cutoff = fish_shape_resolution;
    for (int index = 0; index <= 100; ++index)
    {
        const Real distance_from_head =
            fish_length * (1.0 - Real(index) / 100.0);
        if (outline(distance_from_head, fish_thickness, fish_length) >=
            shape_cutoff)
        {
            return Vec2d(cx + distance_from_head, cy);
        }
    }
    throw std::runtime_error(
        "The selected --dp leaves no resolvable fish-tail observation point");
}
//----------------------------------------------------------------------
...
struct FreeStreamVelocity
{
    ...
    Vecd operator()(Vecd &position, Vecd &velocity, Real current_time)
    {
        ...
        Real time_factor = current_time / t_ref_; // uses `current_time` argument — NOT physical_time_
