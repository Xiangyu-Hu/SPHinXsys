                ...
                for (size_t n = 0; n != inner_neighborhood.current_size_; ++n)
                {
                    ...
                    * if ((vel_[index_i]).dot(e_ij) > 0.0)
                    {    
                        ...
                        K_adv_[index_i] += -(dW_ij * Vol_[index_j] * rho_[index_i]) * (K_[index_j]) * ((vel_[index_i]).dot(e_ij));
                    }
                    else
                    {
                        K_adv_[index_i] += -(dW_ij * Vol_[index_j] * rho_[index_i]) * (K_[index_i]) * ((vel_[index_i]).dot(e_ij));
                    }
        ...
        void KEpsilonStd2ndHalf<RiemannSolverType>::interaction(size_t index_i, Real dt)
        {
            ...
            if (wall_adjacent_cell_flag_[index_i] != 1)
            {
                ...
                for (size_t n = 0; n != inner_neighborhood.current_size_; ++n)
                {
                    ...
                    if ((vel_[index_i]).dot(e_ij) > 0.0)
                    {
                        Eps_adv_[index_i] += -(dW_ij * Vol_[index_j] * rho_[index_i]) * (Eps_[index_j]) * ((vel_[index_i]).dot(e_ij));
                    }
                    else
                    {
                        Eps_adv_[index_i] += -(dW_ij * Vol_[index_j] * rho_[index_i]) * (Eps_[index_i]) * ((vel_[index_i]).dot(e_ij));
                    }
