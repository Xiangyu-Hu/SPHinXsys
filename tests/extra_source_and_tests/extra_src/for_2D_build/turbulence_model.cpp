...
BaseTurbulence::BaseTurbulence(BaseInnerRelation &inner_relation, GhostCreationFromMesh &ghost_creator)
    : BaseIntegration<DataDelegateInner>(inner_relation),
      ...
      K_prod_p_(this->particles_->template registerStateVariableData<Real>("TKEProductionInWallAdjCell")),
      K_prod_(this->particles_->template registerStateVariableData<Real>("TKEProduction")),
      Eps_p_(this->particles_->template registerStateVariableData<Real>("DissipationRateInWallAdjCell")),
      Tau_wall_(this->particles_->template registerStateVariableData<Real>("WallShearStress")),
...
WallAdjacentCells::WallAdjacentCells(BaseInnerRelation &inner_relation, GhostCreationFromMesh &ghost_creator)
    : BaseTurbulence(inner_relation, ghost_creator), wall_normal_(this->particles_->template registerStateVariableData<Vecd>("WallNormal")),
      wall_adjacent_cell_flag_(this->particles_->template registerStateVariableData<Real>("FlagForWallAdjacentCells")),
      yp_(this->particles_->template registerStateVariableData<Real>("WallNormalDistance")),
      ymax_(0.0)
{
    walladjacentcellyp();
}
...
StdWallFunctionFVM::StdWallFunctionFVM(BaseInnerRelation &inner_relation, GhostCreationFromMesh &ghost_creator)
    : BaseTurbulence(inner_relation, ghost_creator), y_star_(this->particles_->template registerStateVariableData<Real>("Ystar")),
      ...
      vel_gradient_mat_(this->particles_->template registerStateVariableData<Matd>("VelocityGradient")),
