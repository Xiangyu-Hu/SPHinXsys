...
inline std::string resolveTeam7ReferenceCsvPath(const std::string &reference_dir)
{
    const StdVec<std::string> candidates = {
        reference_dir + "/TEAM7_Bz_A1_B1_reference_mT.csv",
        reference_dir + "/team7/TEAM7_Bz_A1_B1_reference_mT.csv",
        "TEAM7-reference/reference_data/team7/TEAM7_Bz_A1_B1_reference_mT.csv",
    };
...
{
    probes.clear();
    const std::string csv_path = resolveTeam7ReferenceCsvPath(reference_dir);
    ...
    while (std::getline(input, line))
    {
        ...
        probe.bz_ref_phase0_mT = std::atof(fields[2].c_str());
        probe.position_m = Vecd(probe.x_mm * mm_to_m, y_mm * mm_to_m, z_mm * mm_to_m);
