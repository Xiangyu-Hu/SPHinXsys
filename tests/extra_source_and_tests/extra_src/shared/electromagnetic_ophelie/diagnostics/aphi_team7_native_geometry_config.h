...
struct AphiTeam7NativeGeometryConfig
{
    ...
    /** Generate/relax air SPH particles. EM reload for Ophelie uses coil+plate only. */
    bool relax_air_particles = true;
};
...
inline bool copyTeam7NativeReloadArtifacts(const std::string &from_dir, const std::string &to_dir)
{
    ...
    StdVec<std::string> files = {"Coil_rld.xml", "Plate_rld.xml", "team7_native_geometry.txt"};
    if (fs::exists(fs::path(from_dir) / "Air_rld.xml"))
    {
        files.insert(files.begin() + 2, "Air_rld.xml");
    }
    bool all_ok = true;
...
inline StdVec<std::string> team7NativeReloadCaseLookupOrder(const std::string &case_id)
{
    ...
    else if (case_id == "uniform_small_dp3")
    {
        order.push_back("team7_native_uniform_small_dp3");
    }
    else if (case_id == "team7_native_uniform_small_dp3")
    {
        order.push_back("uniform_small_dp3");
    }
    return order;
}
...
inline bool applyTeam7NativeCasePreset(const std::string &preset, AphiTeam7NativeGeometryConfig &config)
{
    ...
    if (preset == "uniform-small-dp3")
    {
        config.dp_reference = 0.003;
        config.discretization = "uniform";
        config.local_refinement_levels = 0;
        applyTeam7NativeAirPreset("small", config);
        return true;
...
inline void printTeam7NativeGeometryConfigHelp()
{
    std::cout << "TEAM7 native geometry options (SI meters):\n"
              << "  --team7-case=<preset>       uniform-legacy-dp6 | uniform-small-dp6 | uniform-small-dp3 | multires-small-dp6-l1 | multires-small-dp6-l2 | uniform | multires\n"
              ...
              << "  --team7-no-air              skip air particle generation/relax (coil+plate only)\n"
              ...
              << "Reload snapshots: ./reload_cases/uniform_legacy_dp6/ | ./reload_cases/uniform_small_dp3/ | ./reload_cases/multires_small_dp6_l1/ | ./reload_cases/multires_small_dp6_l2/\n";
}
...
inline bool parseTeam7NativeGeometryCli(int ac, char *av[], AphiTeam7NativeGeometryConfig &config)
{
    ...
    for (int i = 1; i < ac; ++i)
    {
        ...
        else if (arg == "--team7-no-air")
        {
            config.relax_air_particles = false;
        }
    }
...
inline bool writeTeam7NativeGeometryMetadata(const std::string &path, const AphiTeam7NativeGeometryConfig &config)
{
    ...
    output << "relax_air_particles," << (config.relax_air_particles ? 1 : 0) << "\n";
    return true;
}
