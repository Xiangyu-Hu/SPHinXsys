    ...
    getExtendedInterfaceState(const FluidStateSecondOrderUpwind &state_i, const FluidStateSecondOrderUpwind &state_j, const Vecd &e_ij, const Real &r_ij)
{
    ...
    Vecd vel_grad_limited = Vecd::Zero();
    if (vel_dot_eij > 0) // Flow from j to i (j is upstream)
    { 
        // Upstream: K_j, Downstream: K_i
        r = (state_j.K_grad_.dot(e_ij)) / (state_i.K_grad_.dot(e_ij));
        // Apply limiter (Minmod)
        phi = std::max(0.0, std::min(1.0, r));
        //phi = (r + std::abs(r)) / (1 + std::abs(r));
        K_grad_limited = phi * state_j.K_grad_;
        k_star = state_j.K_ + 0.5 * K_grad_limited.dot(e_ij) * r_ij;
        Eps_grad_limited = phi * state_j.Eps_grad_;
        eps_star = state_j.Eps_ + 0.5 * Eps_grad_limited.dot(e_ij) * r_ij;
        
        r_vel = state_j.vel_gradient_mat_.sum() / state_i.vel_gradient_mat_.sum();
        phi_vel = std::max(0.0, std::min(1.0, r));
        vel_grad_limited = phi_vel * state_j.vel_gradient_mat_ * e_ij;
        v_star = state_j.vel_ + 0.5 * vel_grad_limited * r_ij;
    }
    else // Flow from i to j (i is upstream)
    {
        // Upstream: K_i, Downstream: K_j
        r = (state_i.K_grad_.dot(-e_ij)) / (state_j.K_grad_.dot(-e_ij)); // Ratio-based r for upstream and downstream
        // Apply limiter (e.g., Minmod)
        phi = std::max(0.0, std::min(1.0, r));
        //phi = (r + std::abs(r)) / (1 + std::abs(r));
        K_grad_limited = phi * state_i.K_grad_;
        // Compute interface value
        k_star = state_i.K_ + 0.5 * K_grad_limited.dot(-e_ij) * r_ij;
        Eps_grad_limited = phi * state_i.Eps_grad_;
        eps_star = state_i.Eps_ + 0.5 * Eps_grad_limited.dot(-e_ij) * r_ij;

        r_vel = state_i.vel_gradient_mat_.sum() / state_j.vel_gradient_mat_.sum();
        phi_vel = std::max(0.0, std::min(1.0, r));
        vel_grad_limited = phi_vel * state_i.vel_gradient_mat_ * (-e_ij);
        v_star = state_i.vel_ + 0.5 * vel_grad_limited * r_ij;
    }
    return ExtendedFluidStarState(
        star_state.rho_,
        v_star,
