/**
 * STAGE 6 PLUMBING SMOKE — NOT a convergence verification.
 * Pure vector Laplace on A (laplace_a only, reaction=false): singular / ill-conditioned.
 * Validates ||R0||=||RHS|| and that BiCGStab iteration loop runs.
 */
...
class AssignVectorLaplaceFieldsCK : public LocalDynamics
{
  public:
    explicit AssignVectorLaplaceFieldsCK(SPHBody &sph_body, const AphiBlockNames &block_names)
...
int main(int ac, char *av[])
{
    ...
    const UnsignedInt max_iterations = 200;
    ...
    StateDynamics<MainExecutionPolicy, AssignVectorLaplaceFieldsCK> assign_exact(test_body.body, names.solution);
    ...
    AphiBiCGStabSolverCK<MainExecutionPolicy> solver(test_body.body, test_body.inner(), names, options, tolerance,
                                                       max_iterations);
    ...
    const AphiBiCGStabResult result = solver.solve();

    BaseParticles &particles = test_body.body.getBaseParticles();
    const size_t total_real_particles = particles.TotalRealParticles();
    const Real host_rhs_norm = hostBlockNorm(particles, names.rhs, total_real_particles);
    const Real initial_norm_error = std::abs(result.initial_residual_norm - host_rhs_norm);

    const bool passed = result.initial_residual_norm > TinyReal && initial_norm_error / host_rhs_norm < 1.0e-4 &&
                        result.iteration_count > 0;

    std::cout << "test_3d_aphi_ck_bicgstab_vector_laplace_singular_smoke"
              << " PLUMBING_SMOKE_ONLY=1"
              << " iterations=" << result.iteration_count
              << " initial_residual_norm=" << result.initial_residual_norm
              << " host_rhs_norm=" << host_rhs_norm
              << " breakdown_code=" << AphiBiCGStabBreakdownCodeName(result.breakdown_code)
