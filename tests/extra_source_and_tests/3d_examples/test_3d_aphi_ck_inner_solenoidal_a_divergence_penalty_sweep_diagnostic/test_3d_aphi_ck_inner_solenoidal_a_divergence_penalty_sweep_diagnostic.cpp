/**
 * Stage 10.7 inner: solenoidal J=curl C source, sweep lambda_A with post-solve divA.
 */
...
int main(int ac, char *av[])
{
    ...
    std::cout << "test_3d_aphi_ck_inner_solenoidal_a_divergence_penalty_sweep_diagnostic begin phi_gauge_penalty="
              << phi_gauge_penalty << std::endl;

    size_t converged_rows = 0;
    for (const SweepCase &sweep_case : sweep_cases)
    {
        const AphiInnerADivergencePenaltySweepRow row = runInnerSolenoidalADivergencePenaltySweepRow(
            ...
            source_region, solenoidal_current_amplitude, imag_to_real_ratio);
        printInnerADivergencePenaltySweepRow(
            "test_3d_aphi_ck_inner_solenoidal_a_divergence_penalty_sweep_diagnostic", row);
        if (row.converged)
        {
            converged_rows += 1;
        }
    ...
    std::cout << "test_3d_aphi_ck_inner_solenoidal_a_divergence_penalty_sweep_diagnostic completed_rows="
