/**
 * Stage 7 convergence (b): vector A complex Helmholtz (laplace_a + reaction, no phi/coupling).
 */
...
class AssignVectorHelmholtzFieldsCK : public LocalDynamics
{
  public:
    explicit AssignVectorHelmholtzFieldsCK(SPHBody &sph_body, const AphiBlockNames &block_names)
...
int main(int ac, char *av[])
{
    ...
    const UnsignedInt max_iterations = 300;
    ...
    StateDynamics<MainExecutionPolicy, AssignVectorHelmholtzFieldsCK> assign_exact(test_body.body, names.solution);
    ...
    const bool passed = result.converged && !result.breakdown &&
                        result.final_residual_norm / result.initial_residual_norm < tolerance;

    std::cout << "test_3d_aphi_ck_bicgstab_vector_helmholtz_convergence"
