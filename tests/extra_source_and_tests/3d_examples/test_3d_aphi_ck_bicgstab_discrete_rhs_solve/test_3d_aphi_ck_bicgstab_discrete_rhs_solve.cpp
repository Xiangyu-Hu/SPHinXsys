 ...
 * Full A-phi operator, X0=0: checks ||R0||=||RHS|| and solver loop execution.
 */
...
int main(int ac, char *av[])
{
    ...
    /** Smoke: verify X0=0 gives R0=RHS norm; full convergence pending gauge/preconditioner (Stage 7). */
    const bool passed = result.initial_residual_norm > TinyReal && initial_norm_error / host_rhs_norm < 1.0e-4 &&
                        result.iteration_count > 0;

    std::cout << "test_3d_aphi_ck_bicgstab_discrete_rhs_solve"
              ...
              << " relative_residual=" << (result.final_residual_norm / result.initial_residual_norm)
              << " breakdown_code=" << AphiBiCGStabBreakdownCodeName(result.breakdown_code)
              << " converged=" << (result.converged ? 1 : 0)
              << " breakdown=" << (result.breakdown ? 1 : 0)
