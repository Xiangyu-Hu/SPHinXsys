/**
 * Stage 9A diagnostic: BiCGStab on scalar phi Laplace + penalty (expected to fail vs GMRES).
 */
...
int main(int ac, char *av[])
{
    ...
    solver_options.solver_kind = AphiKrylovSolverKind::BiCGStabDiagnostic;
    solver_options.bicgstab.relative_tolerance = tolerance;
    solver_options.bicgstab.max_iterations = max_iterations;
    solver_options.bicgstab.use_block_jacobi_preconditioner = true;
    solver_options.bicgstab.recompute_true_residual = true;
    solver_options.bicgstab.true_residual_check_interval = 5;
    ...
    std::cout << "test_3d_aphi_ck_bicgstab_scalar_phi_laplace_penalty_diagnostic"
