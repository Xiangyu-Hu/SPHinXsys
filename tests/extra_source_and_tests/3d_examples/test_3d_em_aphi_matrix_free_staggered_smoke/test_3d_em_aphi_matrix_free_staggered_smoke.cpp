 ...
 * @brief Relation-based source-free manufactured baselines for the matrix-free staggered A-phi solver skeleton.
 */
...
int main(int ac, char *av[])
{
    ...
    const std::string case_mode = readEnvString("EM_APHI_STAGGERED_SMOKE_CASE", "transverse_source_free");
    ...
    if (case_mode == "coulomb_variable_sigma_forced_response")
    {
        for (size_t i = 0; i != number_of_particles; ++i)
        {
            ...
            sources.source_ay[i] = forced_source_amplitude * std::sin(phase_x);
    ...
    std::cout << std::setprecision(12)
              << "matrix_free_aphi_staggered_smoke"
              << " case_mode=" << case_mode
              << " dp=" << dp_0
              << " points=" << number_of_particles
