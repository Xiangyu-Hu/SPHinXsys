/**
 * @file test_3d_em_aphi_matrix_free_constriction_heating.cpp
 * @brief Simple constriction-heating baseline for the matrix-free A-phi to Joule-heating chain.
 */
...
class ConstrictionBandShape : public ComplexShape
{
  public:
    ConstrictionBandShape(const std::string &shape_name, Real body_length, Real body_height,
                          Real constriction_length, Real constriction_height, Real dp_0)
        : ComplexShape(shape_name)
    {
        const Real side_length = 0.5 * (body_length - constriction_length);
        const Real side_half_length = 0.5 * side_length;
        const Real left_center_x = side_half_length;
        const Real right_center_x = body_length - side_half_length;
        const Real center_y = 0.5 * body_height;
        const Real center_z = 0.5 * dp_0;

        add<GeometricShapeBox>(Transform(Vecd(left_center_x, center_y, center_z)),
                               Vecd(side_half_length, 0.5 * body_height, 0.5 * dp_0));
        add<GeometricShapeBox>(Transform(Vecd(right_center_x, center_y, center_z)),
                               Vecd(side_half_length, 0.5 * body_height, 0.5 * dp_0));
        add<GeometricShapeBox>(Transform(Vecd(0.5 * body_length, center_y, center_z)),
                               Vecd(0.5 * constriction_length, 0.5 * constriction_height, 0.5 * dp_0));
    }
};
...
int main(int ac, char *av[])
{
    ...
    const Real requested_constriction_length = readEnvReal("EM_APHI_CONSTRICTION_LENGTH", 0.20 * body_length);
    const Real requested_constriction_height = readEnvReal("EM_APHI_CONSTRICTION_HEIGHT", 0.16);
    const Real constriction_length = SMAX(static_cast<Real>(2.0) * dp_0,
                                          SMIN(requested_constriction_length, static_cast<Real>(0.8) * body_length));
    const Real constriction_height = SMAX(static_cast<Real>(2.0) * dp_0,
                                          SMIN(requested_constriction_height, body_height));
    ...
    SolidBody body(sph_system, makeShared<ConstrictionBandShape>("ConstrictionBody", body_length, body_height, constriction_length, constriction_height, dp_0));
    ...
    size_t neck_particle_count = 0;
    size_t shoulder_particle_count = 0;
    const Real constriction_center_x = 0.5 * body_length;
    const Real constriction_half_length = 0.5 * constriction_length;
    const Real constriction_window_pad = 0.25 * dp_0;
    for (size_t i = 0; i != number_of_particles; ++i)
    {
        const bool in_neck_window =
            std::abs(positions[i][0] - constriction_center_x) <= constriction_half_length + constriction_window_pad;
        if (in_neck_window)
        {
            neck_joule_sum += final_joule_density[i];
            neck_temperature_delta_sum += temperature_delta_field[i];
            ++neck_particle_count;
        }
        else
        {
            shoulder_joule_sum += final_joule_density[i];
            shoulder_temperature_delta_sum += temperature_delta_field[i];
            ++shoulder_particle_count;
        }
    }
    const Real neck_joule_average =
        neck_particle_count == 0 ? 0.0 : neck_joule_sum / static_cast<Real>(neck_particle_count);
    const Real shoulder_joule_average =
        shoulder_particle_count == 0 ? 0.0 : shoulder_joule_sum / static_cast<Real>(shoulder_particle_count);
    const Real neck_temperature_delta_average =
        neck_particle_count == 0 ? 0.0 : neck_temperature_delta_sum / static_cast<Real>(neck_particle_count);
    const Real shoulder_temperature_delta_average = shoulder_particle_count == 0
                                                        ? 0.0
                                                        : shoulder_temperature_delta_sum / static_cast<Real>(shoulder_particle_count);
    const Real joule_neck_to_shoulder_ratio =
        shoulder_joule_average <= TinyReal ? 0.0 : neck_joule_average / shoulder_joule_average;
    const Real temperature_delta_neck_to_shoulder_ratio =
        shoulder_temperature_delta_average <= TinyReal ? 0.0
                                                       : neck_temperature_delta_average / shoulder_temperature_delta_average;
    ...
    const Real validation_min_joule_neck_ratio = readEnvReal("EM_APHI_CONSTRICTION_MIN_JOULE_NECK_RATIO", -1.0);
    const Real validation_min_temperature_neck_ratio =
        readEnvReal("EM_APHI_CONSTRICTION_MIN_TEMPERATURE_NECK_RATIO", -1.0);
    ...
    if (validation_min_joule_neck_ratio >= Real(0.0) &&
        joule_neck_to_shoulder_ratio < validation_min_joule_neck_ratio)
    {
        validation_pass = false;
    }
    if (validation_min_temperature_neck_ratio >= Real(0.0) &&
        temperature_delta_neck_to_shoulder_ratio < validation_min_temperature_neck_ratio)
    {
        validation_pass = false;
    }

    std::cout << std::setprecision(12)
              << "matrix_free_aphi_constriction_heating"
              << " case_mode=" << case_mode
              << " dp=" << dp_0
              << " points=" << number_of_particles
              ...
              << " constriction_height=" << constriction_height
              << " neck_particle_count=" << neck_particle_count
              << " shoulder_particle_count=" << shoulder_particle_count
              << " neck_joule_average=" << neck_joule_average
              << " shoulder_joule_average=" << shoulder_joule_average
              << " neck_temperature_delta_average=" << neck_temperature_delta_average
              << " shoulder_temperature_delta_average=" << shoulder_temperature_delta_average
              << " joule_neck_to_shoulder_ratio=" << joule_neck_to_shoulder_ratio
              << " temperature_delta_neck_to_shoulder_ratio=" << temperature_delta_neck_to_shoulder_ratio
              ...
              << " validation_min_joule_neck_ratio=" << validation_min_joule_neck_ratio
              << " validation_min_temperature_neck_ratio=" << validation_min_temperature_neck_ratio
