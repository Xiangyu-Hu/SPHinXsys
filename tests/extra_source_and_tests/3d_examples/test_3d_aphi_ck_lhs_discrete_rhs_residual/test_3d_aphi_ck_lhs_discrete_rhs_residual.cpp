...
int main(int ac, char *av[])
{
    ...
    StateDynamics<MainExecutionPolicy, AphiComputeResidualCK> compute_residual(test_body.body, names);
    ...
    const Real core_max_residual = coreBlockMaxAbsResidual(particles, names, positions, total_real_particles,
                                                         body_length, body_height, body_width, core_shell);
    const bool passed = core_particles > 0 && core_max_residual < validation_threshold;

    std::cout << "test_3d_aphi_ck_lhs_discrete_rhs_residual"
              << " core_particles=" << core_particles
              << " core_max_residual=" << core_max_residual
