...
# Phase 3 (EM + stirring + thermal, recommended for client demo / video)
./test_3d_ophelie_rh200_glass_em_stirring --reload=1 --run=1 --dp=0.005 \
  --end-time=60 \
  --joule-mode=em-grid --target-power=50000 \
  --state-recording=1
# Default preset rh200-demo-current: T0=373.15 K, mu=0.05 Pa·s (ParaView-friendly baseline).
# Literature molten-glass run: add --preset=rh200-french-like-full (T0=1473 K, mu=4 Pa·s).
...
- Temperature: demo preset T0=**373.15 K**; use `Temperature` directly or `Temperature-373.15`. Literature preset: `Temperature-1473`.
