 ...
 *
 * All bodies use framework AdaptiveBody + AdaptiveNearSurface (air uses distance to coil/plate union).
 * Uniform lattice: --team7-local-refinement-level=0 (same pattern as test_3d_particle_relaxation_sycl).
 * Multires (air only): --team7-local-refinement-level=N; coil/plate always uniform at dp_reference.
 *
 * Examples (published under ./reload_cases/<case_id>/):
 *   Uniform legacy 6 mm:  --team7-case=uniform-legacy-dp6
 *   Multires small L=2:   --team7-case=multires-small-dp6-l2
 *
 * Probe/solve: --team7-reload-case=uniform_legacy_dp6 --team7-write-vtp
 */
...
int main(int ac, char *av[])
{
    ...
    std::cout << "particle_generation_em TEAM7 native relax (SI meters)"
    ...
    Team7AdaptiveCoilBody &coil_body = sph_system.addAdaptiveBody<SolidBody, AphiTeam7NativeAdaptiveNearSurface>(
        solid_adaptation, makeShared<CoilShape>("Coil", config.stl_scale));
    Team7AdaptivePlateBody &plate_body = sph_system.addAdaptiveBody<SolidBody, AphiTeam7NativeAdaptiveNearSurface>(
        solid_adaptation, makeShared<PlateShape>("Plate", config.stl_scale));
    Team7AdaptiveAirBody &air_body = sph_system.addAdaptiveBody<RealBody, AphiTeam7NativeAdaptiveNearInnerSurface>(
        air_adaptation,
    ...
    std::cout << "coil_particles=" << coil_body.getBaseParticles().TotalRealParticles()
              ...
              << " air_particles=" << air_body.getBaseParticles().TotalRealParticles() << std::endl;
    ...
    auto &air_near_surface = air_body.addBodyPart<NearShapeSurface>();
    ...
    StdVec<SPHBody *> all_bodies = {&coil_body, &plate_body, &air_body};
