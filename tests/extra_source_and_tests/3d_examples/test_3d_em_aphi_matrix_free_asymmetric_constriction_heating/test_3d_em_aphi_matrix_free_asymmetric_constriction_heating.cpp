/**
 * @file test_3d_em_aphi_matrix_free_asymmetric_constriction_heating.cpp
 * @brief Asymmetric constriction-heating baseline for the matrix-free A-phi to Joule-heating chain.
 */
...
class AsymmetricConstrictionBandShape : public ComplexShape
{
  public:
    AsymmetricConstrictionBandShape(const std::string &shape_name, Real body_length, Real body_height,
                                    Real split_x, Real narrow_height, Real dp_0)
        : ComplexShape(shape_name)
    {
        const Real wide_length = split_x;
        const Real narrow_length = body_length - split_x;
        const Real center_y = 0.5 * body_height;
        const Real center_z = 0.5 * dp_0;

        add<GeometricShapeBox>(Transform(Vecd(0.5 * wide_length, center_y, center_z)),
                               Vecd(0.5 * wide_length, 0.5 * body_height, 0.5 * dp_0));
        add<GeometricShapeBox>(Transform(Vecd(split_x + 0.5 * narrow_length, center_y, center_z)),
                               Vecd(0.5 * narrow_length, 0.5 * narrow_height, 0.5 * dp_0));
    }
};
...
int main(int ac, char *av[])
{
    ...
    const Real requested_asym_split_x = readEnvReal("EM_APHI_ASYM_SPLIT_X", 0.50 * body_length);
    const Real requested_asym_narrow_height = readEnvReal("EM_APHI_ASYM_NARROW_HEIGHT", 0.12);
    const Real asym_split_x = SMAX(static_cast<Real>(4.0) * dp_0,
                                   SMIN(requested_asym_split_x, body_length - static_cast<Real>(4.0) * dp_0));
    const Real asym_narrow_height = SMAX(static_cast<Real>(2.0) * dp_0,
                                         SMIN(requested_asym_narrow_height, body_height));
    ...
    SolidBody body(sph_system,
                   makeShared<AsymmetricConstrictionBandShape>("AsymmetricConstrictionBody", body_length,
                                                               body_height, asym_split_x, asym_narrow_height, dp_0));
    ...
    if (case_mode != "coulomb_variable_sigma_forced_response")
    {
        std::cerr << "joule_heating_target_only_supports_coulomb_variable_sigma_forced_response" << std::endl;
        return 3;
    }
    ...
    Real neck_joule_sum = 0.0;
    Real shoulder_joule_sum = 0.0;
    Real neck_temperature_delta_sum = 0.0;
    Real shoulder_temperature_delta_sum = 0.0;
    const StdVec<Real> source_weights = [&]() {
        StdVec<Real> weights(number_of_particles, 0.0);
        for (size_t i = 0; i != number_of_particles; ++i)
        {
            weights[i] = std::abs(sources.source_ay[i]);
        }
        return weights;
    }();
    const Real source_centroid_x = computeWeightedCentroidX(source_weights, positions);
    const Real joule_centroid_x = computeWeightedCentroidX(final_joule_density, positions);
    const Real temperature_delta_centroid_x = computeWeightedCentroidX(temperature_delta_field, positions);
    const Real source_centroid_shift = source_centroid_x - 0.5 * body_length;
    const Real joule_centroid_shift = joule_centroid_x - 0.5 * body_length;
    const Real temperature_delta_centroid_shift = temperature_delta_centroid_x - 0.5 * body_length;
    const Real joule_relative_to_source_shift = joule_centroid_x - source_centroid_x;
    const Real temperature_delta_relative_to_source_shift = temperature_delta_centroid_x - source_centroid_x;

    Real narrow_joule_sum = 0.0;
    Real wide_joule_sum = 0.0;
    Real narrow_temperature_delta_sum = 0.0;
    Real wide_temperature_delta_sum = 0.0;
    size_t narrow_particle_count = 0;
    size_t wide_particle_count = 0;
    for (size_t i = 0; i != number_of_particles; ++i)
    {
        const bool in_narrow_side = positions[i][0] >= asym_split_x;
        if (in_narrow_side)
        {
            narrow_joule_sum += final_joule_density[i];
            narrow_temperature_delta_sum += temperature_delta_field[i];
            ++narrow_particle_count;
        }
        else
        {
            wide_joule_sum += final_joule_density[i];
            wide_temperature_delta_sum += temperature_delta_field[i];
            ++wide_particle_count;
        }
    }
    const Real narrow_joule_average =
        narrow_particle_count == 0 ? 0.0 : narrow_joule_sum / static_cast<Real>(narrow_particle_count);
    const Real wide_joule_average =
        wide_particle_count == 0 ? 0.0 : wide_joule_sum / static_cast<Real>(wide_particle_count);
    const Real narrow_temperature_delta_average =
        narrow_particle_count == 0 ? 0.0 : narrow_temperature_delta_sum / static_cast<Real>(narrow_particle_count);
    const Real wide_temperature_delta_average = wide_particle_count == 0
                                                    ? 0.0
                                                    : wide_temperature_delta_sum / static_cast<Real>(wide_particle_count);
    const Real joule_narrow_to_wide_ratio =
        wide_joule_average <= TinyReal ? 0.0 : narrow_joule_average / wide_joule_average;
    const Real temperature_delta_narrow_to_wide_ratio =
        wide_temperature_delta_average <= TinyReal ? 0.0
                                                   : narrow_temperature_delta_average / wide_temperature_delta_average;
    ...
    const Real validation_min_joule_ratio = readEnvReal("EM_APHI_ASYM_MIN_JOULE_RATIO", -1.0);
    const Real validation_min_temperature_ratio = readEnvReal("EM_APHI_ASYM_MIN_TEMPERATURE_RATIO", -1.0);
    const Real validation_min_joule_shift = readEnvReal("EM_APHI_ASYM_MIN_JOULE_SHIFT", -1.0);
    const Real validation_min_temperature_shift = readEnvReal("EM_APHI_ASYM_MIN_TEMPERATURE_SHIFT", -1.0);
    ...
    if (validation_min_joule_ratio >= Real(0.0) &&
        joule_narrow_to_wide_ratio < validation_min_joule_ratio)
    {
        validation_pass = false;
    }
    if (validation_min_temperature_ratio >= Real(0.0) &&
        temperature_delta_narrow_to_wide_ratio < validation_min_temperature_ratio)
    {
        validation_pass = false;
    }
    if (validation_min_joule_shift >= Real(0.0) &&
        joule_centroid_shift < validation_min_joule_shift)
    {
        validation_pass = false;
    }
    if (validation_min_temperature_shift >= Real(0.0) &&
        temperature_delta_centroid_shift < validation_min_temperature_shift)
    {
        validation_pass = false;
    }

    std::cout << std::setprecision(12)
              << "matrix_free_aphi_asymmetric_constriction_heating"
              << " case_mode=" << case_mode
              << " dp=" << dp_0
              << " points=" << number_of_particles
              ...
              << " asym_split_x=" << asym_split_x
              << " asym_narrow_height=" << asym_narrow_height
              << " source_centroid_x=" << source_centroid_x
              << " joule_centroid_x=" << joule_centroid_x
              << " temperature_delta_centroid_x=" << temperature_delta_centroid_x
              << " source_centroid_shift=" << source_centroid_shift
              << " joule_centroid_shift=" << joule_centroid_shift
              << " temperature_delta_centroid_shift=" << temperature_delta_centroid_shift
              << " joule_relative_to_source_shift=" << joule_relative_to_source_shift
              << " temperature_delta_relative_to_source_shift=" << temperature_delta_relative_to_source_shift
              << " narrow_particle_count=" << narrow_particle_count
              << " wide_particle_count=" << wide_particle_count
              << " narrow_joule_average=" << narrow_joule_average
              << " wide_joule_average=" << wide_joule_average
              << " narrow_temperature_delta_average=" << narrow_temperature_delta_average
              << " wide_temperature_delta_average=" << wide_temperature_delta_average
              << " joule_narrow_to_wide_ratio=" << joule_narrow_to_wide_ratio
              << " temperature_delta_narrow_to_wide_ratio=" << temperature_delta_narrow_to_wide_ratio
