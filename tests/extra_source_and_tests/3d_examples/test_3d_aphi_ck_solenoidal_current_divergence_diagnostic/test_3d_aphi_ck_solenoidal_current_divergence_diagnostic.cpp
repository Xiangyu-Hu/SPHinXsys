/**
 * Stage 10.7: solenoidal current source divergence diagnostic (div J_s for J = curl C).
 */
...
int main(int ac, char *av[])
{
    ...
    StateDynamics<MainExecutionPolicy, AssignSolenoidalCurlCurrentRhsCK> assign_solenoidal_current(
    ...
    const Real impressed_reference_div_j_relative = 4.88;
    const bool passed = metrics.source_j_l2 > TinyReal && metrics.source_region_count > 0 &&
                        metrics.source_div_j_relative < impressed_reference_div_j_relative;

    std::cout << "test_3d_aphi_ck_solenoidal_current_divergence_diagnostic"
