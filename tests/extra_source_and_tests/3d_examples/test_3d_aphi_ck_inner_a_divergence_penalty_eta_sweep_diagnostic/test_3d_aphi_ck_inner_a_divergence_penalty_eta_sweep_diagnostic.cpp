/**
 * Stage 10.8: relative eta_A sweep for Inner impressed-current box (pairwise divA diagnostic).
 */
...
int main(int ac, char *av[])
{
    ...
    std::cout << "test_3d_aphi_ck_inner_a_divergence_penalty_eta_sweep_diagnostic begin"
    ...
    {
        const AphiInnerADivergencePenaltyEtaSweepRow row = runInnerImpressedEtaSweepRow(
            ac, av, dp_0, body_length, body_height, body_width, boundary_width, core_shell, sigma, nu, omega,
            phi_gauge_penalty, eta_a, scale_metrics, tolerance, source_region, current_real, current_imag,
            impressed_current_amplitude);
        printInnerADivergencePenaltyEtaSweepRow("test_3d_aphi_ck_inner_a_divergence_penalty_eta_sweep_diagnostic",
                                                row);
        if (row.converged)
        {
            converged_rows += 1;
    ...
    std::cout << "test_3d_aphi_ck_inner_a_divergence_penalty_eta_sweep_diagnostic completed_rows="
