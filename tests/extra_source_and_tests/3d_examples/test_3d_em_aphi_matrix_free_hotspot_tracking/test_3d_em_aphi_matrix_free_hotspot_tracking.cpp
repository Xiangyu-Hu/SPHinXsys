 ...
 * @brief Localized hot-spot tracking baseline for the matrix-free A-phi to Joule-heating chain.
 */
...
int main(int ac, char *av[])
{
    ...
    const Real source_centroid_x = computeWeightedCentroidX(
        [&]() {
    ...
    const Real source_center_error = std::abs(source_centroid_x - forced_gaussian_center);
    const Real joule_center_error = std::abs(joule_centroid_x - forced_gaussian_center);
    const Real temperature_delta_center_error = std::abs(temperature_delta_centroid_x - forced_gaussian_center);
    ...
    const Real validation_max_source_center_error = readEnvReal("EM_APHI_HOTSPOT_MAX_SOURCE_CENTER_ERROR", -1.0);
    const Real validation_max_joule_center_error = readEnvReal("EM_APHI_HOTSPOT_MAX_JOULE_CENTER_ERROR", -1.0);
    const Real validation_max_temperature_center_error = readEnvReal("EM_APHI_HOTSPOT_MAX_TEMPERATURE_CENTER_ERROR", -1.0);
    ...
    if (validation_max_source_center_error >= Real(0.0) && source_center_error > validation_max_source_center_error)
    {
        validation_pass = false;
    }
    if (validation_max_joule_center_error >= Real(0.0) && joule_center_error > validation_max_joule_center_error)
    {
        validation_pass = false;
    }
    if (validation_max_temperature_center_error >= Real(0.0) &&
        temperature_delta_center_error > validation_max_temperature_center_error)
    {
        validation_pass = false;
    }

    std::cout << std::setprecision(12)
              << "matrix_free_aphi_hotspot_tracking"
              << " case_mode=" << case_mode
              << " dp=" << dp_0
              << " points=" << number_of_particles
              ...
              << " source_center_error=" << source_center_error
              << " joule_center_error=" << joule_center_error
              << " temperature_delta_center_error=" << temperature_delta_center_error
              ...
              << " validation_max_source_center_error=" << validation_max_source_center_error
              << " validation_max_joule_center_error=" << validation_max_joule_center_error
              << " validation_max_temperature_center_error=" << validation_max_temperature_center_error
