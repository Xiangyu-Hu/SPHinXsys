/**
 * Stage 9A diagnostic: GMRES with vs without block-Jacobi preconditioner.
 */
...
void runGMRESCase(SPHBody &body, Inner<> &inner, const AphiVariableNames &names, const AphiGMRESWorkspaceNames &workspace,
                  const AphiLhsAssemblyOptions &options, StateDynamics<MainExecutionPolicy, AphiZeroBlockCK> &zero_solution,
                  AphiLhsTestBody &test_body, const char *label, bool use_preconditioner, Real tolerance,
                  UnsignedInt restart_dimension, UnsignedInt max_outer_iterations)
{
    zero_solution.exec();
    ...
    gmres_options.use_block_jacobi_preconditioner = use_preconditioner;

    AphiGMRESSolverCK<MainExecutionPolicy> solver(body, inner, names, workspace, options, gmres_options);
    const AphiGMRESResult result = solver.solve();

    std::cout << " " << label << " outer=" << result.outer_iteration_count << " arnoldi=" << result.arnoldi_step_count
              << " rel_res=" << result.final_relative_residual
              << " true_rel_res=" << result.final_true_relative_residual
              << " recursive_true_gap=" << result.final_recursive_true_gap << " converged=" << (result.converged ? 1 : 0)
...
int main(int ac, char *av[])
{
    ...
    std::cout << "test_3d_aphi_ck_gmres_preconditioner_comparison_diagnostic";
    runGMRESCase(test_body.body, test_body.inner(), names, workspace, options, zero_solution, test_body, "no_pc",
                 false, tolerance, restart_dimension, max_outer_iterations);
    runGMRESCase(test_body.body, test_body.inner(), names, workspace, options, zero_solution, test_body, "block_jacobi",
                 true, tolerance, restart_dimension, max_outer_iterations);
