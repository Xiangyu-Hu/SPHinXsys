/**
 * Stage 8B diagnostic: scalar phi Laplace + phi gauge penalty PCG from zero initial guess.
 * Intentionally fails until the scalar operator/solver path converges; passed=1 only on convergence.
 */
...
int main(int ac, char *av[])
{
    ...
    AphiPCGSolverOptions pcg_options;
    pcg_options.max_iterations = max_iterations;
    pcg_options.relative_tolerance = tolerance;
    pcg_options.use_block_jacobi_preconditioner = true;
    pcg_options.recompute_true_residual = true;
    pcg_options.true_residual_check_interval = 5;
    pcg_options.enable_debug_log = false;

    AphiPCGSolverCK<MainExecutionPolicy> solver(test_body.body, test_body.inner(), names, options, pcg_options);
    ...
    const AphiPCGResult result = solver.solve();
    const bool passed = result.converged && !result.breakdown &&
                        result.final_relative_residual < tolerance;

    std::cout << "test_3d_aphi_ck_pcg_scalar_phi_laplace_penalty_diagnostic"
              ...
              << " breakdown_code=" << AphiPCGBreakdownCodeName(result.breakdown_code)
