...
protected:
    ...
    //ObservedQuantityRecording<Matd> fish_active_strain_probe;
    //----------------------------------------------------------------------
...
public:
    explicit SphFishReloadTrain(int episode_env) :
        ...
        //fish_active_strain_probe("ActiveStrain", fish_observer_contact_fish)
    {
        ...
        //fish_active_strain_probe.writeToFile();
    }
    ...
    {
        imposing_active_strain.setLambfromPython(lambda_from_python);
    }
    ...
    void setAmUPfromPython(Real amup_from_python)
    {
        imposing_active_strain.setAmUPfromPython(amup_from_python);
    }

    void setAmDOWNfromPython(Real amdown_from_python)
    {
        imposing_active_strain.setAmDOWNfromPython(amdown_from_python);
    }
    //    void setLambdafromPython(Real lambda_from_python)
    //    {
    ...
    {
        StdLargeVec<Vecd> *observedQuantity = fish_velocity_probe.getObservedQuantity();
    ...
    //Real getFishActiveStrain(int number)
    //{
    //    StdLargeVec<Matd> *observedQuantity = fish_active_strain_probe.getObservedQuantity();
    //    //return (*fish_active_strain_probe.getObservedQuantity())[number](0, 0);
    //    return (*observedQuantity)[number](0, 0);
    //}
    void runCase(int episode, Real pause_time_from_python)
    {
        ...
        while (GlobalStaticVariables::physical_time_ < pause_time_from_python)
        {
            ...
            while (integration_time < D_Time)
            {
                ...
                //fish_active_strain_probe.writeToFile(number_of_iterations);
...
class SphBasicTestSystemSetting : public SphBasicGeometrySetting
{
  protected:
    BoundingBox system_domain_bounds;
    SPHSystem sph_system;
  ...
  public:
    SphBasicTestSystemSetting(int episode_env) : system_domain_bounds(Vec2d(-DL_sponge - BW, -BW), Vec2d(DL + BW, DH + BW)),
                                                 sph_system(system_domain_bounds, particle_spacing_ref), inlet_particle_buffer(0.5)
    {
        sph_system.setIOEnvironment();
    }
...
class SphFishRelaxationTestEnvironment : public SphBasicTestSystemSetting
{
  protected:
    FluidBody water_block;
    SolidBody fish_body;
  ...
  public:
    SphFishRelaxationTestEnvironment(int episode_env) : SphBasicTestSystemSetting(episode_env),
                                                        water_block(sph_system, makeShared<WaterBlock>("WaterBody")),
                                                        fish_body(sph_system, makeShared<FishBody>("FishBody")),
                                                        fish_observer(sph_system, "FishObserver")
    {
...
class SphFishRelaxationTest : public SphFishRelaxationTestEnvironment
{
  protected:
    InnerRelation fish_inner, water_block_inner;
    ContactRelation water_block_contact, fish_contact, fish_observer_contact_water, fish_observer_contact_fish;
  ...
  public:
    explicit SphFishRelaxationTest(int episode_env) : SphFishRelaxationTestEnvironment(episode_env),
                                                      fish_inner(fish_body),
                                                      water_block_inner(water_block),
                                                      water_block_contact(water_block, {&fish_body}),
                                                      fish_contact(fish_body, {&water_block}),
...
class SphFishReloadTestEnvironment : public SphBasicTestSystemSetting
{
  protected:
    FluidBody water_block;
    SolidBody fish_body;
  ...
  public:
    SphFishReloadTestEnvironment(int episode_env) : SphBasicTestSystemSetting(episode_env),
                                                    water_block(sph_system, makeShared<WaterBlock>("WaterBody")),
                                                    fish_body(sph_system, makeShared<FishBody>("FishBody")),
                                                    fish_observer(sph_system, "FishObserver")
    {
...
class SphFishReloadTest : public SphFishReloadTestEnvironment
{
  ...
  public:
    explicit SphFishReloadTest(int episode_env) : SphFishReloadTestEnvironment(episode_env),
                                                  fish_inner(fish_body), water_block_inner(water_block),
                                                  water_block_contact(water_block, {&fish_body}),
                                                  fish_contact(fish_body, {&water_block}),
                                                  fish_observer_contact_water(fish_observer, {&water_block}),
...
PYBIND11_MODULE(test_2d_flow_stream_around_fish_flap_pybind, m)
{
    ...
    py::class_<SphFishReloadTrain>(m, "from_sph_reload_and_train")
        ...
        .def("SetAmUp", &SphFishReloadTrain::setAmUPfromPython)
        .def("SetAmDown", &SphFishReloadTrain::setAmDOWNfromPython)
        ...
        //.def("GetFishActiveStrain", &SphFishReloadTrain::getFishActiveStrain)
        .def("RunCase", &SphFishReloadTrain::runCase);

    py::class_<SphFishRelaxationTest>(m, "from_sph_relaxation_and_test")
        .def(py::init<const int &>());

    py::class_<SphFishReloadTest>(m, "from_sph_reload_and_test")
        .def(py::init<const int &>())
        .def("SetLambda", &SphFishReloadTest::setLambdafromPython)
        .def("SetFreq", &SphFishReloadTest::setFreqfromPython)
        .def("SetAmUp", &SphFishReloadTest::setAmUPfromPython)
        .def("SetAmDown", &SphFishReloadTest::setAmDOWNfromPython)
        .def("GetFishPressurePoint", &SphFishReloadTest::getFishPressure)
        .def("GetFishVelocityX", &SphFishReloadTest::getFishVelocityX)
        .def("GetFishVelocityY", &SphFishReloadTest::getFishVelocityY)
        .def("GetFishPositionX", &SphFishReloadTest::getFishPositionX)
        .def("GetFishPositionY", &SphFishReloadTest::getFishPositionY)
        //.def("GetFishActiveStrain", &SphFishReloadTest::getFishActiveStrain)
        .def("RunCase", &SphFishReloadTest::runCase);
}
