/**
* @file 	3d_elasticSolid_shell_tangentialCollision_sharpAngle.cpp
* @brief 	This is a benchmark test of the 3D elastic solid->shell almost tangential sharp angle contact/impact formulations.
* @details  We consider the collisoin of an elastic cylinder bouncing in a cylindrical shell box.
...
BoundingBox system_domain_bounds(Vec3d(-DL/2. - BW, -DH/2. - BW, -DW/2. - BW), 
								 Vec3d(DL/2. + BW, DH/2. + BW, 6.*DW + BW));

Real ball_radius = 0.5;			
Real wall_radius = 0.5 * DL;	
Real initial_ball_speed = 4.0;
Vec3d initial_velocity = initial_ball_speed*Vec3d(0.15, 0.15, 1.0);
...
public:
	...
	: ThinStructure(system, body_name)
    {
		...
		Real halflength = 6.*DW;

		body_shape_.add<TriangleMeshShapeCylinder>(SimTK::UnitVec3 (0., 0., 1.), 1.5*ball_radius + BW, halflength, resolution, translation_wall);
        body_shape_.substract<TriangleMeshShapeCylinder>(SimTK::UnitVec3 (0., 0., 1.), 1.5*ball_radius, halflength, resolution, translation_wall);
    }
};
...
public:
	...
	: SolidBody(system, body_name)
	{
		...
		Real halflength = 2. * ball_radius;

		body_shape_.add<TriangleMeshShapeCylinder>(SimTK::UnitVec3 (0., 0., 1.), ball_radius + BW, halflength, resolution, translation_ball);
	}
};
/**
