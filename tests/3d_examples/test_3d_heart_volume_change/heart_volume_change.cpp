...
IndexVector MeshData::get_ids_close_to_surface(const StdLargeVec<Vec3d>& pos_0, const IndexVector& all_surface_ids, Real distance) const
{
	...
	for(auto index: all_surface_ids)
	{
		...
		if(result.distance <= distance) ids.push_back(index);
	}
	return ids;
}
...
IndexVector MeshData::get_ids_close_to_surface(const StdLargeVec<Vec3d>& pos_0, Real distance) const
{
	...
	for (size_t i = 0; i < pos_0.size(); ++i)
	{
		...
		if(result.distance <= distance) ids.push_back(i);
	}
	return ids;
}
...
)
{
	...
	// mesh_offset acounts for potential surface mismatch between ventricle and myocardium mesh
	...
	for(auto index: lv_ids_) lv_pos.push_back(particles_.pos0_[index]);
	for(auto index: rv_ids_) rv_pos.push_back(particles_.pos0_[index]);
