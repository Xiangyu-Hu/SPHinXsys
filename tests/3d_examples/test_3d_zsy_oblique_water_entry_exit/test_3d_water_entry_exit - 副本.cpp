...
StdVec<Vecd> wetting_observer_location =
    {object_center - Vecd(0.0, 0.0, 0.1)}; /**< wetting observation point. */
// 前缘观测点（入水点）
StdVec<Vecd> leading_edge_location = {object_center + Vecd(0.1, 0.0, -0.1)}; /**< Leading edge observation point. */

//----------------------------------------------------------------------
//  汇总输出类（3D版本）- 简化版，只输出力和前缘观测点位置
//----------------------------------------------------------------------
...
{
    std::cout << "Starting particle relaxation for 3D object..." << std::endl;
    ...
    //----------------------------------------------------------------------
    int ite_p = 0;
    int max_iterations = 2000; // 3D可能需要更多迭代
...
int main(int ac, char *av[])
{
    ...
    SolidBodyPartForSimbody structure_system(floating_object,
                                             makeShared<GeometricShapeBox>(Transform(object_center), halfsize_obj));
    ...
    // 设置初始位置和姿态
    SimTK::MobilizedBody::Planar &planar_mob = dynamic_cast<SimTK::MobilizedBody::Planar &>(structure_mob);

    // 设置初始位置
    planar_mob.setQ(state, SimTKVec3(0.0, 0.0, 0.0)); // 初始位置偏移设为0

    // 设置初始速度
    SimTK::Vec3 mobilizer_vel(0.0,initial_velocity[0], initial_velocity[1]); // Z轴旋转设为0
    planar_mob.setU(state, mobilizer_vel);
