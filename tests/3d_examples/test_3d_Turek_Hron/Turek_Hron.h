...
//INSERT BODY
Vecd insert_cylinder_center(2.0, DH /2, DW /2);	
Real insert_cylinder_radius = 0.5;
/** Plate related parameters. */
Real bh = 0.4 * insert_cylinder_radius;
Real bl = 8.0 * insert_cylinder_radius; 
...
SharedPtr<ComplexShape> createFluid()
{
	...
	fluid_shape->subtract<TriangleMeshShapeCylinder>(SimTK::UnitVec3(0,0,1.0), insert_cylinder_radius, DW / 2, resolution, insert_cylinder_center);
	fluid_shape->subtract<TriangleMeshShapeBrick>(Vec3d(bl/2, bh/2 , DW / 2 ), resolution, translation_plate);
	return fluid_shape;
}
//WALL
SharedPtr<ComplexShape> createWall()
{
	...
	wall_shape->subtract<TriangleMeshShapeBrick>(Vec3d(DL / 2 + BW , DH / 2 , DW / 2 + BW), resolution, translation_fluid);
	return wall_shape;
}
...
SharedPtr<ComplexShape> createInsertBody()
{
	...
	insert_body->add<TriangleMeshShapeCylinder>(SimTK::UnitVec3(0,0,1.0), insert_cylinder_radius, DW / 2, resolution, insert_cylinder_center);
	insert_body->add<TriangleMeshShapeBrick>(Vec3d(bl/2, bh/2 , DW / 2 ), resolution, translation_plate);
	return insert_body;
}
...
SharedPtr<ComplexShape> createBeamBaseShape()
{
	...
	constraint->add<TriangleMeshShapeCylinder>(SimTK::UnitVec3(0,0,1.0), insert_cylinder_radius, DW / 2, resolution, insert_cylinder_center);
	constraint->subtract<TriangleMeshShapeBrick>(Vec3d(bl/2, bh/2 , DW / 2 ), resolution, translation_plate);
	return constraint;
}
