...
# Shared implementation for the GPU-paper benchmark launchers.
...
benchmark_bin_directory()
{
    ...
    printf '%s/tests/gpu_paper_benchmarks/verification/%s/bin' \
        "${BUILD_DIR%/}" "${case_dir}"
}
...
run_verification_suite()
{
    ...
    local -a verification_cases=(
        "poiseuille_2d|poiseuille_2d|gpu_paper_poiseuille_2d|coarse standard fine"
        "diffusion_neumann_2d|diffusion_neumann_2d|gpu_paper_diffusion_neumann_2d|coarse standard fine"
        "dambreak_3d|dambreak_3d|gpu_paper_dambreak_3d|standard"
        "fish_fsi_2d|fish_fsi_2d|gpu_paper_fish_fsi_2d|standard"
        "oscillating_beam_2d|oscillating_beam_2d|gpu_paper_oscillating_beam_2d|standard"
        "twisting_column_3d|twisting_column_3d|gpu_paper_twisting_column_3d|standard"
    )
    ...
    for ((repeat = 1; repeat <= REPETITIONS; repeat += 1)); do
        for resolution in s1 s2 s3 s4 s5 s6; do
            run_benchmark "dambreak_3d" "dambreak_3d" \
                "gpu_paper_dambreak_3d" "${resolution}" "${repeat}" \
