...
`SPHINXSYS_BUILD_GPU_PAPER_BENCHMARKS=ON`; they are not registered with CTest.
Targets work in separate host (`SPHINXSYS_USE_SYCL=OFF`) and SYCL
(`SPHINXSYS_USE_SYCL=ON`) build trees.
...
```text
gpu_paper_dambreak_3d --benchmark --resolution=s3 --end-time=20 \
  --run-id=host-s3-01
```

If `--result-dir` is omitted, each executable defaults to
`<build-dir>/benchmark_results` (for example `build-sycl/benchmark_results`),
...
`build-sycl/benchmark_results/` or `build-host/benchmark_results/`); reviewed
...
baseline is the native host build. The GPU scripts default to `build-sycl`, fix
