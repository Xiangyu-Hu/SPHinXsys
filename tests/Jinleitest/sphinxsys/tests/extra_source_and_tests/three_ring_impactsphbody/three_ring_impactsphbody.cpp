...
void three_ring_impact(int resolution_factor_l, int resolution_factor_m, int resolution_factor_s)
{
    ...
    // resolution
    const Real dp_ref = thickness_l / 10.0; // 4 particles in thickness direction
    ...
    ring_s_body.generateParticles<BaseParticles, Lattice>();
    ...
    // Contact relation
    SurfaceContactRelation contact_s(ring_s_body, {&ring_m_body, &ring_l_body});
    SurfaceContactRelation contact_m(ring_m_body, {&ring_s_body, &ring_l_body});
    SurfaceContactRelation contact_l(ring_l_body, {&ring_m_body,&ring_s_body});
    // relaxation
    relax_solid(ring_l_body, ring_l_inner);
    relax_solid(ring_m_body, ring_m_inner);
    ...
    // self contact
    SelfSurfaceContactRelation self_contact_m(ring_m_body);
    ...
    // density
    // self contact
    InteractionDynamics<solid_dynamics::SelfContactFactorSummation> self_contact_density_m(self_contact_m);
    InteractionDynamics<solid_dynamics::SelfContactFactorSummation> self_contact_density_s(self_contact_s);
    // // contact
    ...
    InteractionWithUpdate<solid_dynamics::SelfContactForce>self_contact_forces_s(self_contact_s);
    // contact
    // InteractionWithUpdate<solid_dynamics::ContactForce> contact_forces_s(contact_s);
    // InteractionWithUpdate<solid_dynamics::ContactForce> contact_forces_m(contact_m);
    // InteractionWithUpdate<solid_dynamics::ContactForce> contact_forces_l(contact_l);
    ...
    auto run_simulation = [&]()
    {
        while (physical_time < end_time)
        {
            ...
            ite_output++;
            calculate_stress_s.exec();
            calculate_stress_m.exec();
