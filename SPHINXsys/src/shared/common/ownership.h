...
namespace SPH
{
	...
	CastingType *DynamicCast(OwnerType *owner, CastedType *casted)
	{
		...
		if (tmp == nullptr)//< should most likely be an assert 
		{
	...
	{
		return std::unique_ptr<T>(new T(std::forward<ConstructorArgs>(args)...));//< to replace with std::make_unique when officially setting C++ standard
	}
	...
	class UniquePtrKeeper//< not needed (works out of the box with unique_ptr)
	{
	public:
	 ...
	 */
	template <class BaseType>
	class UniquePtrVectorKeeper//< not needed (works out of the box with unique_ptr)
	{
	public:
	 ...
	 */
	template <class BaseType>
	class SharedPtrKeeper//< not needed (works out of the box with shared_ptr)
	{
	public:
		/** output the observer as pointer */
		template <class DerivedType, typename... ConstructorArgs>
