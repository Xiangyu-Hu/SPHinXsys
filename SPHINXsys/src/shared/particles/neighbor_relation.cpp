...
namespace SPH
{
	...
	{
		neighborhood.j_.push_back(j_index);
		Real weight = distance < kernel_->CutOffRadius(i_h_ratio) ?
			kernel_->W(i_h_ratio, distance, displacement) : 0.0;
	...
	NeighborRelationInnerVariableSmoothingLength::
		NeighborRelationInnerVariableSmoothingLength(SPHBody* body) : NeighborRelation(),
	...
	NeighborRelationContactBodyPart(SPHBody* body, BodyPart* contact_body_part)
		: NeighborRelation(), 
		part_indicator_(*contact_body_part->getBody()->base_particles_->createAVariable<indexInteger, int>("BodyPartByParticleIndicator"))
	{
		Kernel* source_kernel = body->particle_adaptation_->getKernel();
		Kernel* target_kernel = contact_body_part->getBody()->particle_adaptation_->getKernel();
		kernel_ = source_kernel->SmoothingLength() > target_kernel->SmoothingLength() ? source_kernel : target_kernel;

		BodyPartByParticle* contact_body_part_by_particle = dynamic_cast<BodyPartByParticle*>(contact_body_part);
