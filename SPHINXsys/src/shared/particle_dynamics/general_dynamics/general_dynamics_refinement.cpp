...
namespace SPH {
	//=================================================================================================//
	ParticleSplitWithPrescribedArea::ParticleSplitWithPrescribedArea(SPHBody& body, BodyRegionByCell& refinement_area, size_t body_buffer_width) :
	...
	void ParticleSplitWithPrescribedArea::splittingModel(size_t index_i)
	{
		...
		new_indexs_.clear();
		new_indexs_.push_back(index_i);
		new_indexs_.push_back(particle_real_number);
		Vecd pos_splitting = getSplittingPosition(new_indexs_);
	...
	//=================================================================================================//
	Vecd ParticleSplitWithPrescribedArea::getSplittingPosition(StdVec<size_t> new_indexs_)
	{
		srand(int(new_indexs_[0]));
		Real delta_random = 0 + 2.0*Pi*rand() / RAND_MAX * (2.0*Pi - 0);
		Vecd pos_ = particles_->pos_[new_indexs_[0]];
		Real delta = delta_random + Pi;
		Real Vol_split = Vol_[new_indexs_[0]] / 2.0;
	...
	//=================================================================================================//
	Vecd SplitWithMinimumDensityErrorInner::getSplittingPosition(StdVec<size_t> new_index_)
	{
		StdVec<size_t> original_indexs_;
		StdVec<Vecd>new_pos_;
		size_t index_i = new_index_[0];
		original_indexs_.push_back(index_i);
		...
		new_pos_.push_back(pos_i);
		new_pos_.push_back(pos_j);

		Vecd position = compute_density_error.getPositionFromDensityError(original_indexs_, new_pos_, new_index_, 0.2*particle_spacing, 0.65*particle_spacing);
		for (size_t n = 0; n != new_index_.size(); ++n)
			particle_adaptation_->total_split_error_[new_index_[n]] = compute_density_error.density_error_[new_index_[n]];
		return position;
	}
	//=================================================================================================//
	...
	//=================================================================================================//
	ParticleMergeWithPrescribedArea::ParticleMergeWithPrescribedArea(BaseBodyRelationInner& body_inner_relation, BodyRegionByCell& refinement_area) :
		LocalDynamics(body_inner_relation.sph_body_), GeneralDataDelegateInner(body_inner_relation),
		body_(&body_inner_relation.sph_body_), Vol_(particles_->Vol_), pos_(particles_->pos_), rho_(particles_->rho_),
		mass_(particles_->mass_), all_particle_data_(particles_->all_particle_data_),
		body_inner_relation_(&body_inner_relation), vel_n_(particles_->vel_),
		refinement_area_(&refinement_area),
		h_ratio_(*particles_->getVariableByName<Real>("SmoothingLengthRatio"))
	{
		particle_adaptation_ = DynamicCast<ParticleSplitAndMerge>(this, body_->sph_adaptation_);
	...
	void ParticleMergeWithPrescribedArea::interaction(size_t index_i, Real dt)
	{
		...
		if (!tag_merged_[index_i])
			if (mergeCriteria(inner_configuration_[index_i], pos_[index_i], Vol_i)) {
				merge_indexs.push_back(index_i);
				tag_merged_[index_i] = true;
				mergingModel(merge_indexs);
			}
	}
	//=================================================================================================//
			...
			if (!tag_merged_[index_j])
				if (distance_ < search_distance) {
					...
					if (ABS(search_size - particle_adaptation_->ReferenceSpacing()) < TinyReal) {
						if (!resolution_check) {
							merge_indexs.push_back(index_j);
	...
	//=================================================================================================//
	void ParticleMergeWithPrescribedArea::mergingModel(StdVec <size_t> merge_indexs)
	{
		updateMergedParticleInformation(particles_->total_real_particles_, merge_indexs);
		size_t merge_change_number = merge_indexs.size() - 1;
		for (size_t k = merge_change_number; k != -1; --k)
			particles_->copyFromAnotherParticle(merge_indexs[k], particles_->total_real_particles_ - (merge_change_number - k));
		particles_->total_real_particles_ -= merge_change_number;
	}
	//=================================================================================================//
	void ParticleMergeWithPrescribedArea::updateMergedParticleInformation(size_t merged_index, StdVec <size_t> merge_indexs)
	{
		StdVec <Real>merge_mass_;
		Real total_mass = 0.0;
		for (size_t k = 0; k != merge_indexs.size(); ++k)
		{
			merge_mass_.push_back(mass_[merge_indexs[k]]);
			total_mass += mass_[merge_indexs[k]];
		}
		merge_particle_value_(all_particle_data_, merged_index, merge_indexs, merge_mass_);
	...
	{
		compute_density_error.tag_split.clear();
		tag_merged_.clear();
		for (size_t index_i = 0; index_i != particles_->total_real_particles_; ++index_i) {
			tag_merged_.push_back(false);
			compute_density_error.tag_split.push_back(false);
		}
		compute_density_error.initializeDensityError();
	...
	//=================================================================================================//
	void MergeWithMinimumDensityErrorInner::mergingModel(StdVec <size_t> merge_indexs)
	{
		updateMergedParticleInformation(particles_->total_real_particles_, merge_indexs);
		StdVec<size_t> new_index_;
		new_index_.push_back(merge_indexs[0]);
		new_index_.push_back(merge_indexs[merge_indexs.size() - 1]);

		Vecd pos_merging = getMergingPosition(new_index_, merge_indexs);
		StdVec <size_t>new_indexs;
		new_indexs.push_back(merge_indexs[0]);
		new_indexs.push_back(merge_indexs[merge_indexs.size() - 1]);
		updateNewlyMergingParticle(particles_->total_real_particles_, new_indexs, pos_merging);
		kineticEnergyConservation(merge_indexs);

		particles_->copyFromAnotherParticle(merge_indexs[1], particles_->total_real_particles_ - 1);
		particles_->total_real_particles_ -= 1;
	}
	//=================================================================================================//
	Vecd MergeWithMinimumDensityErrorInner::getMergingPosition(StdVec<size_t> new_index_, StdVec <size_t> merge_indexs)
	{
		StdVec<Vecd> new_pos_;
		size_t index_center = new_index_[0];
		Real particle_spacing = pow(Vol_[index_center] / 2.0, 1.0 / (Real)Dimensions);
		Real distance_min = 0.2* particle_spacing;  //angularMomentumConservation(index_center, merge_indexs);
		Real distance_max = 0.65* particle_spacing;

		Vecd position = pos_[merge_indexs[0]] - pos_[index_center];
		Vecd pos_j = pos_[merge_indexs[0]]; // pos_[index_center] + distance_min * position / (position.norm() + TinyReal);
		Vecd pos_i = 2.0*pos_[index_center] - pos_j;
		new_pos_.push_back(pos_i);
		new_pos_.push_back(pos_j);
		compute_density_error.tag_split[new_index_[0]] = true;
		compute_density_error.tag_split[new_index_[1]] = true;

		Vecd position_final = compute_density_error.getPositionFromDensityError(merge_indexs, new_pos_, new_index_, distance_min, distance_max);
		for (size_t n = 0; n != new_index_.size(); ++n)
			particle_adaptation_->total_merge_error_[new_index_[n]] = compute_density_error.density_error_[new_index_[n]];
	...
	void MergeWithMinimumDensityErrorInner::updateNewlyMergingParticle(size_t index_center, StdVec <size_t>new_indexs, Vecd pos_split)
	{
		for (size_t n = 0; n != new_indexs.size(); ++n)
		{
			...
			h_ratio_[new_indexs[n]] = body_->sph_adaptation_->ReferenceSpacing() / particle_spacing_j;
		}
		pos_[new_indexs[0]] = pos_split;
		pos_[new_indexs[1]] = 2.0* pos_[index_center] - pos_[new_indexs[0]];
	}
	//=================================================================================================//
	Real MergeWithMinimumDensityErrorInner::angularMomentumConservation(size_t index_center, StdVec <size_t> merge_indexs)
	{
		rotation = 0.0;
		Real mass = 0.0;
		Real vel_square = 0.0;
		for (size_t n = 0; n != merge_indexs.size(); ++n) {
			Vecd pos = pos_[merge_indexs[n]] - pos_[index_center];
			Vecd vel = vel_n_[merge_indexs[n]] - vel_n_[index_center];
			rotation += mass_[merge_indexs[n]] * (pos[0] * vel[1] - pos[1] * vel[0]);
			mass += mass_[merge_indexs[n]];
			vel_square += mass_[merge_indexs[n]] / mass_[index_center] * vel_n_[merge_indexs[n]].normSqr();
		}
	...
	//=================================================================================================//
	void MergeWithMinimumDensityErrorInner::kineticEnergyConservation(StdVec <size_t>merge_indexs)
	{
		Real E_total = 0.5*(2.0*vel_n_[merge_indexs[0]].normSqr() + vel_n_[merge_indexs[1]].normSqr() + vel_n_[merge_indexs[2]].normSqr());
		Vecd linear_m = 0.5*(2.0*vel_n_[merge_indexs[0]] + vel_n_[merge_indexs[1]] + vel_n_[merge_indexs[2]]);
		Real angular_m = rotation * 2.0;
		if (ABS(pos_[merge_indexs[0]][1]) > TinyReal && ABS(pos_[merge_indexs[0]][0]) > TinyReal) {
			Real cof1 = pos_[merge_indexs[0]][1] / (pos_[merge_indexs[0]][0] + TinyReal);
			Real cof2 = (-linear_m[0] * pos_[merge_indexs[0]][1] + linear_m[1] * pos_[merge_indexs[0]][0] + angular_m) / (2.0*pos_[merge_indexs[0]][0] + TinyReal);
			...
			if ((b*b - 4.0*a*c) >= 0.0)
				vel_n_[merge_indexs[0]][0] = (-b + sqrt(b*b - 4.0*a*c)) / (2.0*a + TinyReal);
			else {
				vel_n_[merge_indexs[0]][0] = -b / (2.0*a + TinyReal);
			}
			vel_n_[merge_indexs[0]][1] = cof1 * vel_n_[merge_indexs[0]][0] + cof2;
		}
		else if (ABS(pos_[merge_indexs[0]][1]) <= TinyReal) //y=0.0
		{
			vel_n_[merge_indexs[0]][1] = 0.5*(angular_m / (pos_[merge_indexs[0]][0] + TinyReal) + linear_m[1]);
			Real a = 2.0;
			Real b = -2.0*linear_m[0];
			Real c = -E_total + linear_m.normSqr() - 2.0* vel_n_[merge_indexs[0]][1] * linear_m[1] + 2.0*vel_n_[merge_indexs[0]][1] * vel_n_[merge_indexs[0]][1];
			if ((b*b - 4.0*a*c) >= 0.0)
				vel_n_[merge_indexs[0]][0] = (-b + sqrt(b*b - 4.0*a*c)) / (2.0*a + TinyReal);
			else {
				vel_n_[merge_indexs[0]][0] = -b / (2.0*a + TinyReal);
		...
		else if (ABS(pos_[merge_indexs[0]][0]) <= TinyReal) //x=0.0
		{
			vel_n_[merge_indexs[0]][0] = 0.5*(-angular_m / (pos_[merge_indexs[0]][1] + TinyReal) + linear_m[0]);
			Real a = 2.0;
			Real b = -2.0*linear_m[1];
			Real c = -E_total + linear_m.normSqr() - 2.0* vel_n_[merge_indexs[0]][0] * linear_m[0] + 2.0*vel_n_[merge_indexs[0]][0] * vel_n_[merge_indexs[0]][0];
			if ((b*b - 4.0*a*c) >= 0.0)
				vel_n_[merge_indexs[0]][1] = (-b + sqrt(b*b - 4.0*a*c)) / (2.0*a + TinyReal);
			else {
				vel_n_[merge_indexs[0]][1] = -b / (2.0*a + TinyReal);
			}
		}
		vel_n_[merge_indexs[1]][0] = linear_m[0] - vel_n_[merge_indexs[0]][0];
		vel_n_[merge_indexs[1]][1] = linear_m[1] - vel_n_[merge_indexs[0]][1];
	}
	//================================================================================================ =//
	Vecd ComputeDensityErrorInner::getPositionFromDensityError(StdVec<size_t> original_indexs_, StdVec<Vecd> new_pos_, 
		StdVec<size_t> new_index_, Real min_distance, Real max_distance)
	{
		size_t index_center = new_index_[0];
		Vecd pos_final = new_pos_[0];
		Vecd pos_iteration = new_pos_[0];
		...
		{
			temp_E = ABS(sigma_E);
			pos_iteration = getPosition(original_indexs_, new_pos_, new_index_);
			...
			if (ABS(ABS(sigma_E) - temp_E) < residual) {
				pos_final = pos_iteration;
				break;
			}
			if (error_sum_min > sigma_E) {
				error_sum_min = ABS(sigma_E);
				pos_final = pos_iteration;
			}
			iteration += 1;
			new_pos_[0] = pos_iteration;
			new_pos_[1] = 2.0*particles_->pos_[index_center] - pos_iteration;
		}
		for (size_t n = 0; n != new_index_.size(); ++n)
			density_error_[new_index_[n]] = error_sum_min;
	...
	//================================================================================================ =//
	Vecd ComputeDensityErrorInner::getPosition(StdVec<size_t> original_indexs_, StdVec<Vecd> new_pos_, StdVec<size_t> new_index_)
	{
		E_cof_sigma = 0.0;
		sigma_E = 0.0;
		E_cof = Vecd(0.0);
		densityErrorOfNewGeneratedParticles(new_index_, new_pos_);
		densityErrorOfNeighborParticles(new_index_, original_indexs_, new_pos_);

		E_cof_sigma += dot(E_cof, E_cof);
		Real cof = 1.0 / (E_cof_sigma + TinyReal);
		Real k = sigma_E * cof;
		Vecd dr = E_cof * k;
		Vecd update_position = new_pos_[0] + dr;

		return update_position;
	}
	//=================================================================================================//
	void ComputeDensityErrorInner::densityErrorOfNewGeneratedParticles(StdVec<size_t> new_index_, StdVec<Vecd> new_pos_)
	{
		sign_newIndex.clear();
		size_t index_rho = new_index_[0];
		Vecd grad_kernel = computeKernelGradient(index_rho);
		for (size_t n = 0; n != new_pos_.size(); ++n) {
			Vecd displacement = new_pos_[n] - particles_->pos_[index_rho];
			Real rho_newIndex = computeNewGeneratedParticleDensity(index_rho, new_pos_[n]);
			Real error = particles_->rho_[index_rho] - rho_newIndex + dot(grad_kernel, -displacement);

			sign_newIndex.push_back(error / (ABS(error) + TinyReal));
			sigma_E += sign_newIndex[n] * error;
		}
		E_cof += sign_newIndex[0] * 2.0*dW_newIndex[0] - sign_newIndex[1] * 2.0*dW_newIndex[1];
		E_cof += sign_newIndex[0] * grad_newIndex[0] - sign_newIndex[1] * grad_newIndex[1];
	}
	//=================================================================================================//
	Vecd ComputeDensityErrorInner::computeKernelGradient(size_t index_rho)
	{
		Neighborhood& inner_neighborhood = body_inner_relation_.inner_configuration_[index_rho];
		Vecd grad_kernel = Vecd(0);
		for (size_t n = 0; n != inner_neighborhood.current_size_; ++n) {
			grad_kernel += inner_neighborhood.dW_ij_[n] * inner_neighborhood.e_ij_[n]
	...
	//=================================================================================================//
	Real ComputeDensityErrorInner::computeNewGeneratedParticleDensity(size_t index_rho, Vecd new_pos)
	{
		...
		Vecd displacement = 2.0 * (new_pos - particles_->pos_[index_rho]);
		dW_newIndex.push_back(computeKernelWeightGradientBetweenParticles(h_newIndex, displacement, Vol_newIndex));
		...
		grad_newIndex.push_back(grad_sigma);
		sigma_newIndex = sigma_newIndex * particles_->rho0_* inv_sigma_0;
		return sigma_newIndex;
	}
	//=================================================================================================//
	void ComputeDensityErrorInner::computeDensityErrorOnNeighborParticles(Neighborhood& neighborhood, size_t index_rho, StdVec<size_t> original_indexs_, StdVec<Vecd> new_pos_)
	{
		Real Vol_newIndex = particles_->Vol_[index_rho] / 2.0;
		Real h_newIndex = pow(particles_->Vol_[index_rho] / Vol_newIndex, 1.0 / (Real)Dimensions);
		...
		for (size_t k = 0; k != neighborhood.current_size_; ++k) {
			...
			for (size_t n = 0; n != original_indexs_.size(); ++n) {
				Real h_ratio_min = SMIN(h_ratio_j, h_ratio_[original_indexs_[n]]);
				Vecd displacement = pos_j - particles_->pos_[original_indexs_[n]];
				Real Vol_ratio = particles_->Vol_[original_indexs_[n]] / Vol_j;
			...
			sigma_E += sign * sigma_split_j; 
			E_cof += sign * (grad_sigma_j[1] - grad_sigma_j[0]); 
			E_cof_sigma += dot(sigma_co_j, sigma_co_j); 
		}
	}
	//=================================================================================================//
	void ComputeDensityErrorInner::densityErrorOfNeighborParticles(StdVec<size_t> new_index_, StdVec<size_t> original_indexs_, StdVec<Vecd> new_pos_)
	{
		Neighborhood& neighborhood = body_inner_relation_.inner_configuration_[new_index_[0]];
		computeDensityErrorOnNeighborParticles(neighborhood, new_index_[0], original_indexs_, new_pos_);
	}
	//================================================================================================ =//
	...
	Vecd ComputeDensityErrorWithWall::computeKernelGradient(size_t index_rho)
	{
		...
		for (size_t k = 0; k != complex_relation_.contact_bodies_.size(); ++k)
		{
			...
			for (size_t n = 0; n != contact_neighborhood.current_size_; ++n) {
				grad_kernel += contact_neighborhood.dW_ij_[n] * contact_neighborhood.e_ij_[n]
					* Vol_j[contact_neighborhood.j_[n]];
			}
		}
		return grad_kernel;
	}
	//=================================================================================================//
	void ComputeDensityErrorWithWall::densityErrorOfNeighborParticles(StdVec<size_t> new_index_, StdVec<size_t> original_indexs_, StdVec<Vecd> new_pos_)
	{
		ComputeDensityErrorInner::densityErrorOfNeighborParticles(new_index_, original_indexs_, new_pos_);
		for (size_t k = 0; k != complex_relation_.contact_bodies_.size(); ++k)
		{
	...
	//=================================================================================================//
	Real ComputeDensityErrorWithWall::computeNewGeneratedParticleDensity(size_t index_rho, Vecd new_pos)
	{
		Real Vol_newIndex = particles_->Vol_[index_rho] / 2.0;
		Real h_newIndex = pow(particles_->Vol_[index_rho] / Vol_newIndex, 1.0 / (Real)Dimensions);
		...
		grad_newIndex[grad_newIndex.size() - 1] += grad_sigma;
