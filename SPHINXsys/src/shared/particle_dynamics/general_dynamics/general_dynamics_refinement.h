...
namespace SPH
{
	...
	typedef DataDelegateInner<SPHBody, BaseParticles> GeneralDataDelegateInner;
	...
	{
	public:
		ComputeDensityErrorInner(BaseBodyRelationInner& body_inner_relation) :
			GeneralDataDelegateSimple(body_inner_relation.sph_body_),
			body_inner_relation_(body_inner_relation),
		...
		Vecd getPositionFromDensityError(StdVec<size_t> original_indexs_, StdVec<Vecd> new_pos_, StdVec<size_t> new_indexs_, Real min_distance, Real max_distance);
	...
	protected:
		...
		BaseBodyRelationInner& body_inner_relation_;
		...
		virtual Vecd getPosition(StdVec<size_t> original_indexs_, StdVec<Vecd> new_pos_, StdVec<size_t> new_index_);
		virtual void densityErrorOfNewGeneratedParticles(StdVec<size_t> new_index_, StdVec<Vecd> new_pos_);
		virtual void densityErrorOfNeighborParticles(StdVec<size_t> new_index_, StdVec<size_t> original_indexs_, StdVec<Vecd> new_pos_);
		...
		virtual void computeDensityErrorOnNeighborParticles(Neighborhood& neighborhood, size_t index_rho, StdVec<size_t> original_indexs_, StdVec<Vecd> new_pos_);
	...
	/**
	  ...
	  */
	class ComputeDensityErrorWithWall :
		public ComputeDensityErrorInner
	{
	public:
		ComputeDensityErrorWithWall(ComplexBodyRelation &complex_relation) :
			...
			complex_relation_(complex_relation)
		{
			...
			{
				contact_Vol_.push_back(&(complex_relation_.contact_bodies_[k]->base_particles_->Vol_));
			}
		};
	...
	protected:
		...
		virtual void densityErrorOfNeighborParticles(StdVec<size_t> new_index_, StdVec<size_t> original_indexs_, StdVec<Vecd> new_pos_) override;
	...
	/**
	  ...
	  */
	class ParticleSplitWithPrescribedArea :
		public LocalDynamics, public GeneralDataDelegateSimple
	{
	public:
	...
	protected:
		...
		virtual Vecd getSplittingPosition(StdVec<size_t> new_indexs_);
	...
	*/
	...
	{
	public:
		SplitWithMinimumDensityErrorInner(BaseBodyRelationInner& body_inner_relation, BodyRegionByCell& refinement_area, size_t body_buffer_width) :
	...
	protected:
		...
		virtual Vecd getSplittingPosition(StdVec<size_t> new_indexs_) override;
		virtual void setupDynamics(Real dt) override;
	};
	...
	*/
	...
	{
	public:
		SplitWithMinimumDensityErrorWithWall(ComplexBodyRelation &complex_relation, BodyRegionByCell& refinement_area, size_t body_buffer_width) :
	...
	{
	public:
		ParticleMergeWithPrescribedArea(BaseBodyRelationInner& body_inner_relation, BodyRegionByCell& refinement_area);
	...
	protected:
		...
		{
			void operator () (ParticleData& particle_data, size_t merged_index, StdVec <size_t> merge_indexs, StdVec <Real> merge_mass)
			{
				Real total_mass = 0.0;
				...
				for (size_t i = 0; i != std::get<type_index>(particle_data).size(); ++i) {
					...
					for (size_t k = 0; k != merge_indexs.size(); ++k) 
						particle_data_temp +=merge_mass[k] * (*std::get<type_index>(particle_data)[i])[merge_indexs[k]];
	...
	{
	public:
		MergeWithMinimumDensityErrorInner(BaseBodyRelationInner& body_inner_relation, BodyRegionByCell& refinement_area) :
	...
	protected:
		...
		virtual Vecd getMergingPosition(StdVec<size_t> new_index_, StdVec <size_t> merge_indexs);
		virtual Real angularMomentumConservation(size_t index_center, StdVec <size_t> merge_indexs);
		virtual void kineticEnergyConservation(StdVec <size_t> merge_indexs);
	...
	{
	public:
		MergeWithMinimumDensityErrorWithWall(ComplexBodyRelation &complex_relation, BodyRegionByCell& refinement_area) :
