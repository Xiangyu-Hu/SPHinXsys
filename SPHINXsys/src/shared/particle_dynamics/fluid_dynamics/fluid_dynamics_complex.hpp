	...
	namespace fluid_dynamics
	{
    	...
    	DensitySummation<DensitySummationInnerType>::
            DensitySummation(ComplexBodyRelation* body_complex_relation) :
				DensitySummation(body_complex_relation->inner_relation_, 
       ...
       //=================================================================================================//
		...
		ExtendPressureRelaxation<BasePressureRelaxationType>::
            ExtendPressureRelaxation(BaseBodyRelationType* base_body_relation,
				BaseContactBodyRelation* wall_contact_relation, Real penalty_strength) :
				PressureRelaxation<BasePressureRelaxationType>(base_body_relation, wall_contact_relation),
				penalty_strength_(penalty_strength),
				non_cnsrv_dvel_dt_(*(this->particles_->template createAVariable<indexVector, Vecd>("NonConservativeAcceleration"))) {}
        //=================================================================================================//
        template<class BasePressureRelaxationType>
        ...
        void ExtendPressureRelaxation<BasePressureRelaxationType>::Interaction(size_t index_i, Real dt)
		{
			...
			Real rho_i = this->rho_n_[index_i];
			Real penalty_pressure = fabs(this->p_[index_i]);
			Vecd acceleration(0);
			for (size_t k = 0; k < FluidWallData::contact_configuration_.size(); ++k)
			{
				Real particle_spacing_j1 = 1.0 / FluidWallData::contact_bodies_[k]->particle_adaptation_->ReferenceSpacing();
        ...
        //=================================================================================================//
        template<class BasePressureRelaxationType>
        Vecd ExtendPressureRelaxation<BasePressureRelaxationType>::computeNonConservativeAcceleration(size_t index_i)
		{
