...
namespace SPH
{
	...
	DampingBySplittingComplex<DataTypeIndex, VariableType>::
		DampingBySplittingComplex(ComplexBodyRelation* body_complex_relation,
			std::string variable_name, Real eta) :
		DampingBySplittingInner<DataTypeIndex,VariableType>(body_complex_relation->inner_relation_, variable_name, eta),
		DissipationDataContact(body_complex_relation->contact_relation_)
	{
		...
		DampingBySplittingComplex<DataTypeIndex, VariableType>::computeErrorAndParameters(size_t index_i, Real dt)
	{
		...
		for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
		{
			...
			for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
			{
				...
				Real parameter_b = 2.0 * this->eta_ * contact_neighborhood.dW_ij_[n]
	...
	DampingBySplittingWithWall<DataTypeIndex, VariableType, BaseDampingBySplittingType>::
		DampingBySplittingWithWall(ComplexBodyRelation* body_wall_relation,
			std::string variable_name, Real eta) :
		BaseDampingBySplittingType<DataTypeIndex, VariableType>(body_wall_relation->inner_relation_, variable_name, eta),
		DissipationDataWithWall(body_wall_relation->contact_relation_)	
	{
	...
	DampingPairwiseComplex<DataTypeIndex, VariableType>::
		DampingPairwiseComplex(ComplexBodyRelation* body_complex_relation,
			std::string variable_name, Real eta) :
		DampingPairwiseInner<DataTypeIndex, VariableType>(body_complex_relation->inner_relation_, variable_name, eta),
		DissipationDataContact(body_complex_relation->contact_relation_)
	{
		for (size_t k = 0; k != contact_particles_.size(); ++k)
		{
	...
	void DampingPairwiseComplex<DataTypeIndex, VariableType>::Interaction(size_t index_i, Real dt)
	{
		...
		for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
		{
			...
			for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
			{
				...
				VariableType variable_derivative = (variable_i - variable_k[index_j]);
				parameter_b[n] = this->eta_ * contact_neighborhood.dW_ij_[n]
					* Vol_i * Vol_k[index_j] * dt / contact_neighborhood.r_ij_[n];
	...
	DampingPairwiseWithWall<DataTypeIndex, VariableType, BaseDampingPairwiseType>::
		DampingPairwiseWithWall(ComplexBodyRelation* body_wall_relation,
			std::string variable_name, Real eta) :
		BaseDampingPairwiseType<DataTypeIndex, VariableType>(body_wall_relation->inner_relation_, variable_name, eta),
		DissipationDataWithWall(body_wall_relation->contact_relation_)
	{
		for (size_t k = 0; k != DissipationDataWithWall::contact_particles_.size(); ++k)
		{
		...
		Interaction(size_t index_i, Real dt)
	{
		...
		for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
		{
			...
			for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
			{
				...
				this->variable_[index_i] += parameter_b[n] * (variable_i - variable_k[index_j])
					/ (mass_i - 2.0 * parameter_b[n]);
			}
			//backward sweep
			for (size_t n = contact_neighborhood.current_size_; n != 0; --n)
			{
				...
				//only update particle i
				this->variable_[index_i] += parameter_b[n - 1] * (variable_i - variable_k[index_j])
					/ (mass_i - 2.0 * parameter_b[n - 1]);
			}
		}
	}
	//=================================================================================================//
	...
	DampingWithRandomChoice<DampingAlgorithmType>::
		...
		Real random_ratio, std::string variable_name, Real eta) :
