...
namespace SPH
{
	...
	public:
		...
		{
			Real smoothing_length = body->particle_adaptation_->ReferenceSmoothingLength();
	...
	protected:
		void getDiffusionChangeRateContact(
			size_t particle_i, 
			size_t particle_j, 
			Vecd& e_ij, 
			Real surface_area_ij, 
			StdVec<StdLargeVec<Real>>& species_n_k
			)
		{
			for (size_t m = 0; m < species_diffusion_.size(); ++m)
			{
	...
	public:
		RelaxationOfAllDiffussionSpeciesComplex(ComplexBodyRelation* body_complex_relation) :
			RelaxationOfAllDiffussionSpeciesInner<BodyType, BaseParticlesType, BaseMaterialType>(body_complex_relation->inner_relation_),
			DiffusionReactionDataDelegateContact<BodyType, BaseParticlesType, BaseMaterialType,
			ContactBodyType, ContactBaseParticlesType, ContactBaseMaterialType>(body_complex_relation->contact_relation_),
