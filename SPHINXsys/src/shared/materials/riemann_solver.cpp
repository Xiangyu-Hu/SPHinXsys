...
namespace SPH {
		...
		getPStar(const CompressibleFluidState& state_i, const CompressibleFluidState& state_j, const Vecd& e_ij)
	{
		...
		Real p_star = 0.0;
		if (0.0 < s_l) { p_star = state_i.p_; }
		if (s_l <= 0.0 && s_r >= 0.0) { p_star = state_i.p_ + state_i.rho_*(s_l - ul)*(s_star - ul); }
		if (s_r < 0.0) { p_star = state_j.p_; }
		...
		getVStar(const CompressibleFluidState& state_i, const CompressibleFluidState& state_j, const Vecd& e_ij)
	{
		...
		Real s_star = (state_j.rho_ * ur * (s_r - ur) + state_i.rho_ * ul * (ul - s_l) + state_i.p_ - state_j.p_) / (state_j.rho_ * (s_r - ur) + state_i.rho_ * (ul - s_l));
		Vecd v_star(0);
		if (0.0 < s_l) { v_star = state_i.vel_; }
		if (s_l <= 0.0 && 0.0 <= s_star) { v_star = state_i.vel_ - e_ij * (s_star - ul); }
		if (s_star <= 0.0 && 0.0 <= s_r) { v_star = state_j.vel_ - e_ij * (s_star - ur); }
		if (s_r < 0.0) { v_star = state_j.vel_; }
		...
		getRhoStar(const CompressibleFluidState& state_i, const CompressibleFluidState& state_j, const Vecd& e_ij)
	{
		...
		Real s_star = (state_j.rho_ * ur * (s_r - ur) + state_i.rho_ * ul * (ul - s_l) + state_i.p_ - state_j.p_) / (state_j.rho_ * (s_r - ur) + state_i.rho_ * (ul - s_l));
		Real rho_star = 0.0;
		if (0.0 < s_l) { rho_star = state_i.rho_; }
		if (s_l <= 0.0 && 0.0 <= s_star) { rho_star = state_i.rho_ * (s_l - ul) / (s_l - s_star); }
		if (s_star <= 0.0 && 0.0 <= s_r) { rho_star = state_j.rho_ * (s_r - ur) / (s_r - s_star); }
		if (s_r < 0.0) { rho_star = state_j.rho_; }
		...
		getEStar(const CompressibleFluidState& state_i, const CompressibleFluidState& state_j, const Vecd& e_ij)
	{
		...
		if (0.0 < s_l) { energy_star = state_i.E_; }
		...
		if (s_r < 0.0) { energy_star = state_j.E_; }
