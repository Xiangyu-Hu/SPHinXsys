...
namespace SPH
{
	...
	protected:
		...
		Arrayi number_of_grid_points_{Arrayi::Zero()}; 	/**< number of grid points by dimension */
	public:
		BaseMesh() = default;
		explicit BaseMesh(Arrayi number_of_grid_points);
		BaseMesh(Vecd mesh_lower_bound, Real grid_spacing, Arrayi number_of_grid_points);
		...
		Arrayi NumberOfGridPoints() { return number_of_grid_points_; };
		/** Given the cell number, return the mesh number. */
		Arrayi NumberOfGridPointsFromNumberOfCells(const Arrayi &number_of_cells) { return number_of_cells + Arrayi::Ones(); };
		/** Given the grid point number, return the cell number. */
		Arrayi NumberOfCellsFromNumberOfGridPoints(const Arrayi &number_of_grid_points) { return number_of_grid_points - Arrayi::Ones(); };
		...
		Arrayi transfer1DtoMeshIndex(const Arrayi &number_of_mesh_indexes, size_t i);
		/** Transfer mesh index to 1D int.  */
		size_t transferMeshIndexTo1D(const Arrayi &number_of_mesh_indexes, const Arrayi &mesh_index);
	...
	{
	protected:
		Arrayi number_of_cells_{Arrayi::Zero()}; 	/**< number of cells by dimension */
	...
	public:
		...
		Mesh(Vecd mesh_lower_bound, Arrayi number_of_cells, Real grid_spacing);
		virtual ~Mesh(){};

		/** Return number of cell in each direction, i.e., x-, y- and z-axis.*/
		Arrayi NumberOfCells() { return number_of_cells_; };
