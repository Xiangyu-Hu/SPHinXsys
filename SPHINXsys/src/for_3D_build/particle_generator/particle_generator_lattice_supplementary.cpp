...
namespace SPH {
	...
	void ParticleGeneratorLattice::CreateBaseParticles(BaseParticles* base_particles)
	{
		...
		for (size_t i = 0; i < number_of_lattices_[0]; ++i)
			for (size_t j = 0; j < number_of_lattices_[1]; ++j) 
				for (size_t k = 0; k < number_of_lattices_[2]; ++k) {
					...
					if (body_shape_->checkNotFar(particle_location, lattice_spacing_))
					{
	...
	{
		Real sigma(0);
		Real cutoff_radius = sph_body_->kernel_->GetCutOffRadius();
		int search_range = int(cutoff_radius / sph_body_->particle_spacing_) + 1;
			...
			for (int j = -search_range; j <= search_range; ++j)
				for (int i = -search_range; i <= search_range; ++i)
				{
					...
					if (particle_location.norm() < cutoff_radius)
						sigma += sph_body_->kernel_->W(particle_location);
				}
		return sigma;
	}
	//===============================================================//
