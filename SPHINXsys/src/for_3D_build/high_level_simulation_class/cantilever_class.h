...
public:
	...
	/** topology */
    InnerBodyRelation* myocardium_body_inner;
    ...
    DampingWithRandomChoice<DampingPairwiseInner<indexVector, Vec3d>>* muscle_damping;
...
void PassiveCantilever::initialize_simulation(){
    /** Setup the system. */
    system = new SPHSystem(BoundingBox(Vecd(-SL - BW, -BW, -BW), Vecd(PL + BW, PH + BW, PH + BW)), resolution_ref);
	/** Define the external force. */
    ...
    /** topology */
    myocardium_body_inner = new InnerBodyRelation(myocardium_body);
	...
	muscle_damping = new DampingWithRandomChoice<DampingPairwiseInner<indexVector, Vec3d>>(myocardium_body_inner, 0.1, "Velocity", physical_viscosity);
