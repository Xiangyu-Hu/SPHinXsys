...
// ---- Helper functions copied from Palace main.cpp ----
...
// ---- High-level JSON wrapper ----

int RunPalaceFromJson(const std::string &json_file)
{
  const char *argv_cstr[3];
  argv_cstr[0] = "sphinxsys_palace";
  argv_cstr[1] = json_file.c_str();
  argv_cstr[2] = nullptr;

  return palace_main_embed(2, const_cast<char **>(argv_cstr));
}
